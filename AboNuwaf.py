...
def user_keyboard(user_id=None):
    ...
    rows = [
        ...
        ["⚙️ جدول التشغيل التلقائي"],
        [notify_btn, night_btn],
        ["🛠 الدعم الفني"],
        ["📞 تواصل مع الأدمن"],
        ["الخروج"]
    ]
...
def admin_keyboard(user_id=None):
    ...
    if perm("schedule"):
        rows.append(["⚙️ جدول التشغيل التلقائي"])
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        for uid, uinfo in user_data.items():
            station = uinfo.get("temp_station", "")
            station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == station), "غير معروف")
        ...
        save_settings()
        await query.answer("✅ تم " + ("تفعيل" if auto_cleanup_enabled else "إيقاف") + " التنظيف التلقائي")
        ...
        await query.answer()
        global broadcast_stop_notify
        broadcast_stop_notify = not broadcast_stop_notify
        save_settings()
        await query.answer("✅ تم " + ("تفعيل" if broadcast_stop_notify else "إيقاف") + " إشعار الإيقاف")
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        ...
        await query.edit_message_text(
            ...
            f"📊 التقرير الأسبوعي: {weekly_status}\n"
            f"📦 Backup عند مستخدمين: {backup_status}</blockquote>",
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("📊 التقرير الأسبوعي: " + ("إيقاف" if weekly_report_enabled else "تفعيل"), callback_data="toggle_weekly_report")],
                [InlineKeyboardButton("📦 Backup عند مستخدمين: " + ("إيقاف" if backup_on_new_users else "تفعيل"), callback_data="toggle_backup_threshold")],
        ...
        save_settings()
        await query.answer("✅ تم " + ("تفعيل" if weekly_report_enabled else "إيقاف") + " التقرير الأسبوعي")
    ...
    try:
        ...
        if int(user_id) in banned_users:
            ...
            await message.reply_text(
                ...
                "لفك الحظر تواصل مع المطور:</blockquote>",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("💬 تواصل مع الأدمن", url="https://t.me/AboNuwaf", disable_web_page_preview=True)],
                    [InlineKeyboardButton("🤖 تواصل عبر البوت", url="https://t.me/AboNuwaf1_bot")]
                ])
            )
            return
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            await message.reply_text(
                ...
                reply_markup=ReplyKeyboardMarkup([["إلغاء"]], resize_keyboard=True, disable_web_page_preview=True)
            )       
        elif text == "بدء البث":
            ...
            if not is_admin(int(user_id)) or not has_perm(int(user_id), "stats"):
                await message.reply_text("<blockquote>❌ ليس لديك صلاحية هذا الأمر</blockquote>")
                return
            # نبني الإحصائيات من user_data مباشرة
            active = []
            for uid, uinfo in user_data.items():
                station = uinfo.get("temp_station", "")
                station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == station), "غير معروف")
            ...
            await message.reply_text(
                ...
                f"اختر أدمناً لتعديل صلاحياته:</blockquote>",
                reply_markup=InlineKeyboardMarkup(
                    buttons + [
            ...
            try:
                ...
                if num < 1:
                    raise ValueError
                global broadcast_fail_threshold
...
f"📊 التقرير الأسبوعي: {weekly_status} — {DAYS_AR.get(weekly_report_day, "")} {weekly_report_time}\n"
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "⏹ إشعار الإيقاف: " + ("إيقاف" if broadcast_stop_notify else "تفعيل"),
                        callback_data="toggle_stop_notify"
                    )],
                    [InlineKeyboardButton(
                        f"⚠️ حد الفشل: {broadcast_fail_threshold} مرات",
                        callback_data="set_fail_threshold"
                    )],
                    [InlineKeyboardButton(
                        "📊 التقرير الأسبوعي: " + ("إيقاف" if weekly_report_enabled else "تفعيل"),
                        callback_data="toggle_weekly_report"
                    )],
                    [InlineKeyboardButton(
                        "📦 Backup عند مستخدمين: " + ("إيقاف" if backup_on_new_users else "تفعيل"),
                        callback_data="toggle_backup_threshold"
                    )],
                    [InlineKeyboardButton(
                        f"🔢 عدد المستخدمين: {backup_user_threshold}",
                        callback_data="set_backup_threshold"
                    )],
                ])
            )
