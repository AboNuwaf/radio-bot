...
def user_night_mode_thread():
    """تطبيق الوضع الليلي الشخصي لكل مستخدم"""
    switched_users = set()
    while True:
        current_time = time.strftime("%H:%M")
        for uid, user_info in list(user_data.items()):
            if not user_info.get("night_mode_enabled", False):
                if uid in switched_users:
                    switched_users.discard(uid)
                continue
            start = user_info.get("night_mode_start", "23:00")
            end = user_info.get("night_mode_end", "05:00")
            station_id = user_info.get("night_mode_station", "1")
            station = ST_TIMO.get(station_id, ST_TIMO.get("1"))
            if not station:
                continue
            # تحقق لو دلوقتي وقت الليل
            in_night = False
            if start <= end:
                in_night = start <= current_time < end
            else:  # يعدي منتصف الليل
                in_night = current_time >= start or current_time < end
            if in_night and uid not in switched_users:
                # غير لإذاعة الليل
                ...
                switched_users.add(uid)
                save_data()
                logger.info(f"User {uid} switched to night mode: {station['name']}")
            elif not in_night and uid in switched_users:
                switched_users.discard(uid)
        time.sleep(30)
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        for i, ch in enumerate(REQUIRED_CHANNELS, 1):
            ...
            text += f"📝 النص: {preview}</blockquote>"
        ...
        try:
            await query.message.reply_text(
                ...
                f"👥 المستخدمين: {len(user_data)}\n📢 قنوات الاشتراك: {len(REQUIRED_CHANNELS)}</blockquote>"
            )
            files = [
                ("/data/user_data.json",               "👥 بيانات المستخدمين"),
                ("/data/settings.json",                "⚙️ الإعدادات"),
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("🎙 تغيير المحطة", callback_data=f"unight_station_{uid}")],
            ])
        )
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            REQUIRED_CHANNELS.append({"id": channel_id_add, "url": channel_url_add, "custom_text": "", "added_by": str(user_id)})
            save_required_channels()
            user_state.pop(user_id, None)
            await message.reply_text(
            ...
            if not active:
                ...
                return
            stats_text = f"<blockquote>📊 إحصائيات البث ({len(active)} بث نشط):\n\n"
            for a in active:
                duration = get_broadcast_duration(a["channel"])
                stats_text += f"📡 {a['channel']}\n"
                stats_text += f"🎙 {a['station']}\n"
                stats_text += f"👤 {a['user_id']}\n"
                stats_text += "▬▬▬▬▬▬▬▬▬▬\n"
