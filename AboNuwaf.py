...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        for i, ch in enumerate(REQUIRED_CHANNELS, 1):
            ...
            text = (
                f"<blockquote>📡 {ch_name}\n"
                f"👥 الأعضاء: {members_text}\n"
                f"👤 أضافها: {adder_name} {adder_username}\n"
                f"📝 النص: {preview}</blockquote>"
            )

            buttons = []
            row = []
            if adder_name and adder_username:
                adder_url = f"https://t.me/{adder_username.strip('(@)')}"
                row.append(InlineKeyboardButton(f"👤 {adder_name}", url=adder_url))
            if ch_url:
                row.append(InlineKeyboardButton(f"📢 دخول القناة", url=ch_url))
            if row:
                buttons.append(row)
