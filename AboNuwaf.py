...
def save_settings():
    ...
    try:
        settings = {
            ...
            "broadcast_log": broadcast_log[-100:] if len(broadcast_log) > 100 else broadcast_log,
        }
...
def load_settings():
    ...
    try:
        ...
        if _saved_log and not broadcast_log:
            broadcast_log = _saved_log
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        try:
            ...
            ttype = parts[2] if len(parts) > 2 else "sched"
        ...
        selected_station = parts[2] if len(parts) > 2 else ""
        ...
        if "process" in channel_info and is_ffmpeg_running(channel_info["process"]):
            ...
            if channel_info.get("broadcast_type") == "auto":
                ...
                await query.edit_message_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ نعم، ابدأ البث اليدوي", callback_data=f"confirm_manual|{channel_id}|{selected_station}")
                    ]])
                )
            else:
                await query.answer(
