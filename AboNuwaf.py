...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8628254857:AAHhn9qbZQtEP8ZebmGoWLDGx-2FmX6SGeo")
    ...
    while True:
        ...
        if current_time == "08:00" and not sent_today:
            if daily_report_enabled:
                ...
                try:
                    asyncio.run_coroutine_threadsafe(
                        app.send_message(ADMIN_ID[0], report), app.loop
                    )
                except Exception as e:
...
async def stop_user_broadcasts(user_id):
    ...
    if uid not in user_data:
        return []
    stopped = []
    for channel_id, channel_info in user_data[uid].get("channels", {}).items():
        if "process" in channel_info:
            try:
                pid = channel_info["process"]
                if is_ffmpeg_running(pid):
                ...
                pass
            del user_data[uid]["channels"][channel_id]["process"]
        ...
        try:
            ...
            for uid in active_users:
                ...
                future = asyncio.run_coroutine_threadsafe(
                    _check_and_punish(uid), app.loop
                )
                try:
    ...
    while True:
        ...
        if current_time in AUTO_SCHEDULE and current_time not in schedule_disabled:
            ...
            if switched:
                ...
                asyncio.run_coroutine_threadsafe(
                    notify_schedule_change(station["name"]), app.loop
                )
        time.sleep(60)
...
async def handle_callback(client, query):
    ...
    # اختيار محطة
        ...
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        ch_buttons = []
        for num, (ch_id, ch_info) in enumerate(channels.items()):
...
async def start_command(client, message):
    try:
        ...
        if not is_admin(message.from_user.id) and max_users_enabled:
            if len(user_data) >= max_users_limit and str(message.from_user.id) not in user_data:
                await message.reply_text(
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            # حساب الإحصائيات
            total_users = len(user_data)
            total_channels = sum(len(u.get("channels", {})) for u in user_data.values())
            ...
            try:
                ...
                if not is_admin(int(user_id)) and broadcast_notify_enabled:
                    try:
                        ...
                        pass
                # تحديث إحصائيات البث وعداد الوقت
