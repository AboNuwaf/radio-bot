...
async def check_subscription(client, user_id):
    """التحقق من الاشتراك الحقيقي مع الحماية من رشق الانضمام الوهمي"""
    for channel in REQUIRED_CHANNELS:
        try:
            # تحويل الـ ID لـ integer
            ch_id = channel["id"]
            try:
                ch_id = int(ch_id)
            ...
            # التحقق من الحالة الحقيقية للعضوية
            if member.status.name in ["LEFT", "BANNED", "KICKED", "RESTRICTED"]:
                return channel
            # حماية: التأكد من أن العضو غير مقيد
            if hasattr(member, 'is_member') and member.is_member is False:
                return channel
        except Exception as e:
            err = str(e).lower()
            # لو البوت مش أدمن أو مش عضو في القناة — نتجاهل هذه القناة
            if "not enough rights" in err or "chat_admin_required" in err:
                pass
            # لو المستخدم مش موجود في القناة أصلاً
            elif "user_not_participant" in err or "user_id_invalid" in err:
                return channel
            # لو القناة مش موجودة أو البوت مش فيها — نتجاهل
            elif "channel_invalid" in err or "chat_id_invalid" in err or "peer_id_invalid" in err:
                pass
            else:
                # أي خطأ تاني — نتجاهل ولا نمنع المستخدم
...
async def send_subscription_message(message, channel):
    ...
    await message.reply_text(
        text,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 اشترك الآن", url=channel_url)]
        ])
    )
...
def build_ffmpeg_cmd(input_url: str, output_url: str) -> list:
    """بناء أمر ffmpeg — audio only محسّن للاستقرار"""
...
def admin_keyboard(user_id=None):
    ...
    if perm("notifications"):
        rows.append([channel_notify_btn, notify_btn])
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        await query.edit_message_text(
            ...
            "@username رابط\n\n"
            "أو مع نص مخصص:\n"
            "@username رابط | النص المخصص</blockquote>"
        )
...
@app.on_message(filters.command("testsub") & filters.private)
async def testsub_command(client, message):
    """اختبار الاشتراك الإجباري"""
    if message.from_user.id != OWNER_ID:
        return
    result = "<blockquote>🔍 اختبار الاشتراك الإجباري\n\n"
    for channel in REQUIRED_CHANNELS:
        ch_id = channel["id"]
        try:
            ch_id_int = int(ch_id)
            ...
            ch_id_int = ch_id
        result += f"📢 القناة: {ch_id}\n"
        try:
            # جرب تجيب معلومات القناة
            chat = await client.get_chat(ch_id_int)
            result += f"✅ القناة موجودة: {chat.title}\n"
            # جرب تتحقق من المطور نفسه
            member = await client.get_chat_member(ch_id_int, message.from_user.id)
            result += f"👤 حالتك: {member.status.name}\n"
        except Exception as e:
            result += f"❌ خطأ: {e}\n"
        result += "\n"
    result += "</blockquote>"
...
@app.on_message(filters.command("checksub") & filters.private)
async def checksub_command(client, message):
    """فحص قنوات الاشتراك الإجباري"""
    if message.from_user.id != OWNER_ID:
        return
    import json as _json
    result = f"<blockquote>📢 قنوات الاشتراك الإجباري\n\n"
    result += f"في الذاكرة: {len(REQUIRED_CHANNELS)} قناة\n"
    for i, ch in enumerate(REQUIRED_CHANNELS, 1):
        result += f"{i}. ID: {ch.get('id')} | URL: {ch.get('url','')[:30]}\n"
    result += "\n"
    # شوف الملف
    import os
    if os.path.exists("/data/required_channels_backup.json"):
        with open("/data/required_channels_backup.json") as f:
            data = _json.load(f)
        result += f"في الملف: {len(data)} قناة\n"
        for i, ch in enumerate(data, 1):
            result += f"{i}. ID: {ch.get('id')} | URL: {ch.get('url','')[:30]}\n"
    else:
        result += "❌ الملف مش موجود\n"
    result += "</blockquote>"
    await message.reply_text(result)

@app.on_message(filters.command("testrtmps") & filters.private)
async def testrtmps_command(client, message):
    """اختبار الـ rtmps URL مباشرة"""
    if message.from_user.id != OWNER_ID:
        return
    uid = str(OWNER_ID)
    if uid not in user_data or not user_data[uid].get("channels"):
        await message.reply_text("<blockquote>❌ مش لاقي قنوات</blockquote>")
        return
    channel_info = list(user_data[uid]["channels"].values())[0]
    rtmps_url = channel_info.get("rtmps_url", "")
    station_url = "https://qurango.net/radio/saad_alghamdi"
    await message.reply_text(f"<blockquote>🔧 بيجرب البث لمدة 5 ثواني...\n🔗 {rtmps_url[:60]}...</blockquote>")
    import asyncio
    try:
        proc = await asyncio.create_subprocess_exec(
            "ffmpeg", "-reconnect", "1", "-reconnect_streamed", "1",
            "-reconnect_delay_max", "5", "-re", "-i", station_url,
            "-tls_verify", "0",
            "-vn", "-c:a", "aac", "-b:a", "128k", "-ar", "44100",
            "-loglevel", "verbose", "-t", "5",
            "-f", "flv", rtmps_url,
        ...
        try:
            ...
            err_text = stderr.decode(errors='replace')[-800:]
            await message.reply_text(
                f"<blockquote>{'✅ نجح' if exit_code == 0 else '❌ فشل'} (exit: {exit_code})\n\n{err_text}</blockquote>"
            )
        except asyncio.TimeoutError:
            proc.kill()
            await message.reply_text("<blockquote>⏰ انتهى الوقت بدون error — البث شغال!</blockquote>")
    except Exception as ex:
        await message.reply_text(f"<blockquote>❌ خطأ: {ex}</blockquote>")

@app.on_message(filters.command("checkprocess") & filters.private)
async def checkprocess_command(client, message):
    """أمر /checkprocess — للمطور فقط، يتحقق من processes الشغالة"""
    if message.from_user.id != OWNER_ID:
        return
    uid = str(OWNER_ID)
    result = "<blockquote>🔍 فحص البثوث\n\n"
    
    # شوف كل قنوات المطور
    if uid in user_data:
        channels = user_data[uid].get("channels", {})
        for ch_id, ch_info in channels.items():
            title = ch_info.get("title", ch_id)
            pid = ch_info.get("process")
            running = is_ffmpeg_running(pid) if pid else False
            result += f"📡 {title}\n"
            result += f"   PID: {pid}\n"
            result += f"   شغال: {'✅' if running else '❌'}\n\n"
    
    # شوف كل processes ffmpeg على السيرفر
    try:
        ps_out = subprocess.check_output(["ps", "aux"], text=True, timeout=5)
        ffmpeg_procs = [l for l in ps_out.splitlines() if 'ffmpeg' in l]
        result += f"🖥 كل processes ffmpeg على السيرفر: {len(ffmpeg_procs)}\n"
        for p in ffmpeg_procs[:3]:
            result += f"  {p[:100]}\n"
    except Exception as ex:
        result += f"❌ خطأ في ps: {ex}\n"
    
    result += "</blockquote>"
    await message.reply_text(result)
async def testffmpeg_command(client, message):
    """أمر /testffmpeg — للمطور فقط، يجرب ffmpeg على رابط معين"""
    if message.from_user.id != OWNER_ID:
        return
    await message.reply_text("<blockquote>🔧 جاري اختبار ffmpeg...</blockquote>")
    try:
        # 1. تحقق إن ffmpeg موجود
        result = subprocess.run(["ffmpeg", "-version"], capture_output=True, text=True, timeout=10)
        ffmpeg_version = result.stdout.split('\n')[0] if result.returncode == 0 else "ffmpeg غير موجود!"
        
        # 2. جرب تشغيل ffmpeg على رابط إذاعة لمدة 5 ثواني
        test_url = "https://qurango.net/radio/saad_alghamdi"
        test_out = "/data/test_output.mp3"
        test_cmd = ["ffmpeg", "-y", "-reconnect", "1", "-reconnect_streamed", "1",
                    "-reconnect_delay_max", "5", "-i", test_url,
                    "-t", "5", "-vn", "-c:a", "mp3", "-loglevel", "verbose",
                    test_out]
        proc = subprocess.run(test_cmd, capture_output=True, text=True, timeout=30)
        
        # 3. اجمع النتيجة
        stderr_last = proc.stderr[-1000:] if proc.stderr else "لا يوجد output"
        success = proc.returncode == 0 and os.path.exists(test_out) and os.path.getsize(test_out) > 0
        
        report = (
            f"<blockquote>🔧 نتيجة اختبار ffmpeg\n\n"
            f"📌 الإصدار: {ffmpeg_version}\n"
            f"{'✅ الاختبار نجح' if success else '❌ الاختبار فشل'}\n\n"
            f"📋 آخر سطور الـ output:\n{stderr_last[-500:]}</blockquote>"
        )
        await message.reply_text(report)
        
        # لو نجح ابعت الملف الصوتي
        if success:
            await app.send_audio(OWNER_ID, test_out, caption="✅ صوت تجريبي من ffmpeg")
            os.remove(test_out)
            
    except subprocess.TimeoutExpired:
        await message.reply_text("<blockquote>⏰ انتهى الوقت — ffmpeg مش بيرد</blockquote>")
    except Exception as e:
        await message.reply_text(f"<blockquote>❌ خطأ: {e}</blockquote>")

@app.on_message(filters.command("teststream") & filters.private)
async def teststream_command(client, message):
    """أمر /teststream — للمطور فقط، يجرب البث على الـ rtmps url الحقيقي"""
    if message.from_user.id != OWNER_ID:
        return
    # ابحث عن أول قناة عند المطور
    uid = str(OWNER_ID)
    if uid not in user_data or not user_data[uid].get("channels"):
        await message.reply_text("<blockquote>❌ مش لاقي قنوات عندك — ضيف قناة الأول</blockquote>")
        return
    channels = user_data[uid]["channels"]
    channel_id, channel_info = list(channels.items())[0]
    rtmps_url = channel_info.get("rtmps_url", "")
    station_url = user_data[uid].get("temp_station", "https://qurango.net/radio/saad_alghamdi")
    
    await message.reply_text(
        f"<blockquote>🔧 جاري اختبار البث...\n"
        f"📡 القناة: {channel_info.get('title','')}\n"
        f"🎙 الإذاعة: {station_url[:50]}\n"
        f"🔗 rtmps: {rtmps_url[:50]}...</blockquote>"
    )
    
    try:
        test_cmd = [
            "ffmpeg", "-reconnect", "1", "-reconnect_streamed", "1",
            "-reconnect_delay_max", "5", "-fflags", "+genpts",
            "-re", "-i", station_url,
            "-tls_verify", "0",
            "-vn", "-c:a", "aac", "-b:a", "128k", "-ar", "44100",
            "-loglevel", "verbose",
        ...
        stderr_last = proc.stderr[-800:] if proc.stderr else "لا يوجد"
        await message.reply_text(
            f"<blockquote>{'✅ نجح' if success else '❌ فشل'} (exit code: {proc.returncode})\n\n"
            f"📋 آخر الـ output:\n{stderr_last[-600:]}</blockquote>"
        )
    except subprocess.TimeoutExpired:
        await message.reply_text("<blockquote>⏰ انتهى الوقت — البث مش بيرد</blockquote>")
    except Exception as e:
        await message.reply_text(f"<blockquote>❌ خطأ: {e}</blockquote>")
    ...
    try:
        ...
        if not is_admin(int(user_id)):
            try:
                ...
                # لو حصل خطأ في التحقق نكمل بدون إيقاف المستخدم
        ...
        if text == "إضافة قناة":
            ...
            # إضافة قناة من الزرار
            rest = text.strip()
            if " | " in rest:
                parts_add = rest.split(None, 1)
                if len(parts_add) < 2:
                    await message.reply_text("<blockquote>❌ صيغة غير صحيحة\nمثال: @channel رابط | النص</blockquote>")
                    return
                channel_id_add = parts_add[0]
                remainder = parts_add[1]
                channel_url_add, custom_text_add = remainder.split(" | ", 1)
                channel_url_add = channel_url_add.strip()
                custom_text_add = custom_text_add.strip()
            else:
                parts_add = rest.split(None, 1)
                if len(parts_add) < 2:
                    await message.reply_text("<blockquote>❌ صيغة غير صحيحة\nمثال: @channel رابط</blockquote>")
                    return
                channel_id_add = parts_add[0]
                channel_url_add = parts_add[1].strip()
                custom_text_add = ""
            # نحول الـ username لـ numeric id عشان get_chat_member يشتغل صح
            try:
                chat_obj = await client.get_chat(channel_id_add)
                channel_id_add = str(chat_obj.id)
            except Exception as ex:
                logger.error(f"sub_add get_chat error: {ex}")
                await message.reply_text(
                    "<blockquote>❌ تعذر الوصول إلى القناة\nتأكد أن البوت عضو فيها وأن المعرف صحيح</blockquote>"
                )
                return
            for ch in REQUIRED_CHANNELS:
                if ch["id"] == channel_id_add:
                    ...
                    user_state.pop(user_id, None)
                    return
            REQUIRED_CHANNELS.append({"id": channel_id_add, "url": channel_url_add, "custom_text": custom_text_add})
            save_required_channels()
            user_state.pop(user_id, None)
            txt = f"<blockquote>✅ تم إضافة القناة بنجاح.\n🆔 ID: {channel_id_add}"
            if custom_text_add:
                preview = custom_text_add[:50] + "..." if len(custom_text_add) > 50 else custom_text_add
                txt += f"\n📝 النص: {preview}"
            txt += "</blockquote>"
            await message.reply_text(txt, reply_markup=admin_keyboard(user_id))
