        ...
        try:
            ...
            for uid, user_info in list(user_data.items()):
                ...
                for channel_id, channel_info in list(channels.items()):
                    ...
                    if not station_url:
                        continue
                    # المشكلة 6: لو المستخدم وقفه يدوياً - لا تشغّله
                    if channel_info.get("externally_stopped", False):
                        continue
                    try:
...
def _draw_ch_sched(ch_id, ch_info):
    ...
    # ━━━ زر التراويح ━━━
    tarawih_ar = to_ar(tarawih_time if tarawih_time else "20:30")
    tarawih_icon = "✅" if tarawih_enabled else "❌"
    tarawih_btn = "⏹ إيقاف" if tarawih_enabled else "▶️ تشغيل"
    tarawih_name = TARAWIH_STATION["name"].replace("إذاعة ", "").strip()
    time_buttons.append([InlineKeyboardButton(f"{tarawih_icon} {tarawih_ar}  |  {tarawih_name}", callback_data="noop")])
    time_buttons.append([InlineKeyboardButton(tarawih_btn, callback_data="tarawih_toggle")])
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        ch_title = ch_info.get("title", ch_id)
        # المشكلة 5: لو في بث شغال وعايز يفعّل الوضع الليلي - رسالة تأكيد
        if not current:
            pid = ch_info.get("process")
            has_active = pid and is_ffmpeg_running(pid)
            btype = ch_info.get("broadcast_type", "manual")
            if has_active and btype != "night":
                ...
                await query.edit_message_text(
                    f"<blockquote>⚠️ يوجد بث شغّال على قناة\n📢 {ch_title}\n"
        ...
        if not current:
            # امسح externally_stopped عشان يسمح للوضع الليلي يشغّل البث
            user_data[uid]["channels"][ch_id].pop("externally_stopped", None)
        ...
        if current:
            ...
            await app.send_message(_uid_night_int, "🏠 اللوحة الرئيسية", reply_markup=_main_kb_night)
        else:
            txt, kb = _draw_unight_sched(uid, ch_id, ch_info)
            await query.edit_message_text(txt, reply_markup=kb)
        ...
        await query.edit_message_text(
            f"<blockquote>🗑 هل تريد حذف القناة؟\n📢 {ch_title}</blockquote>",
            reply_markup=InlineKeyboardMarkup([
        ...
        # ━━━ إيقاف الجدول ━━━
            ...
            await app.send_message(_owner_int, "🏠 اللوحة الرئيسية", reply_markup=_main_kb_sched)
        ...
        if "process" in channel_info and is_ffmpeg_running(channel_info["process"]):
            from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
            if channel_info.get("broadcast_type") == "auto":
                ...
                # حفظ الـ station في user_state عشان callback_data ميتعداش 64 بايت
