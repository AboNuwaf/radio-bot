...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_scat_{uid}_{ch_id}_{CAT_INDEX.get(cat_name, cat_name)}")])
        await query.edit_message_text(
        ...
        buttons = []
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_edit_cat_{uid}_{ch_id}_{t}_{CAT_INDEX.get(cat_name, cat_name)}")])
        await query.edit_message_text(
        ...
        # قد يكون uid_ch_id_cat أو uid_cat
        # نحاول نستخرج الـ cat من الآخر
        # format: uid_chid_catindex
        parts = rest.split("_", 2)
        uid = parts[0]
        ch_id_from_cat = parts[1] if len(parts) > 1 else None
        ...
        # format: uid_chid_HH:MM
        t = rest[-5:]  # HH:MM
        rest2 = rest[:-6]  # uid_chid
        ...
        # format: action_chid_HH:MM — الوقت دايماً آخر 5 حروف
        t = rest[-5:]  # HH:MM
        rest2 = rest[:-6]  # action_chid
        parts = rest2.split("_", 1)
        action = parts[0]
        ch_id = parts[1] if len(parts) > 1 else ""
        await query.answer()
        if action == "edit_time":
            ...
            buttons = []
            for cat_name in ST_CATEGORIES.keys():
                buttons.append([InlineKeyboardButton(cat_name, callback_data=f"ch_sched_st_cat_{ch_id}_{t}_{CAT_INDEX.get(cat_name, cat_name)}")])
            await query.edit_message_text(
                f"<blockquote>🎙 تغيير محطة الساعة {t}\n\nاختر القسم:</blockquote>",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
        ...
        # format: action_uid_chid_HH:MM
        t = rest[-5:]  # HH:MM
        rest2 = rest[:-6]  # action_uid_chid
        parts = rest2.split("_", 2)
        action = parts[0]
        ...
        buttons = []
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"ch_sched_add_cat_{ch_id}_{CAT_INDEX.get(cat_name, cat_name)}")])
        await query.edit_message_text(
        ...
        parts = rest.split("_", 1)
        ch_id = parts[0]
        cat_raw = parts[1] if len(parts) > 1 else ""
        cat = CAT_BY_INDEX.get(cat_raw, cat_raw)
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        station_ids = ST_CATEGORIES.get(cat, [])
            ...
            if text == "➕ إضافة محطة":
                ...
                for cat_name in ST_CATEGORIES.keys():
                    if mode == "ch_sched":
                        ...
                        buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_add_cat_{uid}_{ch_id}_{CAT_INDEX.get(cat_name, cat_name)}")])
                await message.reply_text(
