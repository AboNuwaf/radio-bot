...
def save_broadcast_state():
    ...
    for user_id, user_info in list(user_data.items()):
        ...
        for channel_id, channel_info in list(channels.items()):
            if "process" in channel_info:
                # استخدم station القناة نفسها مش temp_station المشترك
    ...
    try:
        ...
        for user_id, user_info in current_data.items():
            ...
            for channel_id, info in list(channels.items()):
                ...
                if not selected_station:
                    # لو مفيش station محفوظة اتجاهل القناة دي
            ...
            for user_id, user_info in list(user_data.items()):
                ...
                for channel_id, channel_info in list(channels.items()):
                    ...
                    if not is_ffmpeg_running(pid):
                        ...
                        # إشعار المستخدم بإيقاف البث من البوت
                        try:
                            ch_title = channel_info.get("title", channel_id)
                            asyncio.run_coroutine_threadsafe(
                                app.send_message(
                                    int(uid),
                                    f"<blockquote>⚠️ تم اكتشاف إيقاف البث على قناة\n"
                                    f"📢 {ch_title}\n\n"
                                    f"• يرجى إيقاف البث من البوت أيضاً لتجنب أي أخطاء تحدث لقناتك.</blockquote>"
                                ), _bot_loop
                            )
                        except Exception:
                            pass
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        # اسم المحطة من station القناة نفسها
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            channels = user_data[user_id]["channels"]
            # فحص القنوات النشطة
            active_channels = {ch_id: ch_info for ch_id, ch_info in channels.items() if "process" in ch_info}
            if not active_channels:
                await message.reply_text("<blockquote>ℹ️ لا يوجد بث نشط حالياً</blockquote>")
                return
            # لو قناة واحدة نشطة - اوقفها مباشرة
            ...
            for channel_id, info in list(channels.items()):
                ...
                # اسم المحطة من station كل قناة لوحدها
