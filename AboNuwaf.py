...
def save_data():
    ...
    # trigger backup لو فات أكتر من ساعة من آخر backup
    global _last_auto_backup
    if _bot_loop and time.time() - _last_auto_backup > 3600:
        try:
    ...
    if data == "clear_reviews":
        ...
        try:
            if "process" in channel_info:
                ...
                # المشكلة 6: أوقف الجدول/الوضع الليلي مع البث
                if was_auto:
...
async def _auto_backup_user_data():
    """حفظ user_data تلقائياً في قناة البيانات عند كل تغيير مهم"""
    global _last_auto_backup
    try:
        # احفظ في ملف أولاً
        with data_lock:
        ...
        # بعت required_channels كمان لو موجود
        ...
        logger.info(f"✅ Auto backup user_data sent to {BACKUP_CHANNEL_ID}")
        return msg
    except Exception as e:
...
async def _auto_restore_user_data():
    ...
    try:
        ...
        # استعادة required_channels كمان لو مش موجود
        if not REQUIRED_CHANNELS:
            try:
                async for message in app.get_chat_history(BACKUP_CHANNEL_ID, limit=50):
                    if (message.document and 
                        message.document.file_name == "required_channels_backup.json"):
                        rc_path = "/data/required_channels_backup.json"
                        await message.download(file_name=rc_path)
                        load_required_channels()
                        logger.info(f"✅ تم استعادة required_channels تلقائياً — {len(REQUIRED_CHANNELS)} قناة")
                        break
            except Exception as e:
                logger.error(f"auto_restore required_channels error: {e}")
        return True
    except Exception as e:
...
def _auto_backup_thread():
    """thread يبعت backup كل 30 دقيقة لو في تغييرات"""
    _last_count = 0
    while True:
        try:
            time.sleep(1800)  # 30 دقيقة
            current_count = len(user_data)
            if current_count != _last_count or time.time() - _last_auto_backup > 3600:
                if _bot_loop:
                    asyncio.run_coroutine_threadsafe(_auto_backup_user_data(), _bot_loop)
