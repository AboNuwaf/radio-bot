...
# تحويل اسم الكاتيجوري لرقم مختصر في الـ callback
CAT_INDEX = {name: str(i) for i, name in enumerate(ST_CATEGORIES.keys())}
CAT_BY_INDEX = {str(i): name for i, name in enumerate(ST_CATEGORIES.keys())}
                    ...
                    for uid, uinfo in list(user_data.items()):
                        for ch_id, ch_info in list(uinfo.get("channels", {}).items()):
                            try:
                                ...
                                ch_info["broadcast_type"] = "auto"
                                ch_info["should_broadcast"] = True
                                reset_broadcast_stats(channel_id, TARAWIH_STATION["name"], uid, "auto")
                            except Exception as e:
        ...
        try:
            ...
            for uid, user_info in list(user_data.items()):
                ...
                for channel_id, channel_info in list(channels.items()):
                    ...
                    if not station_url:
                        continue
                    # لو المستخدم وقف البث يدوياً - لا تشغّله
                    if channel_info.get("should_broadcast") is False:
                        continue
                    try:
                        ...
                        # المشكلة 6: broadcast_type = night
                        user_data[uid]["channels"][channel_id]["broadcast_type"] = "night"
                        user_data[uid]["channels"][channel_id].pop("externally_stopped", None)
                        user_data[uid]["channels"][channel_id]["should_broadcast"] = True
                    except Exception as e:
                    ...
                    # المشكلة 3: تصفير وقت البدء عند تغيير إذاعة الوضع الليلي
                    reset_broadcast_stats(channel_id, station_name, uid, "night")
                    # المشكلة 4: إضافة للسجل
                    uinfo_n = user_data.get(uid, {})
                    broadcast_log.append({
                        "time": egypt_strftime("%Y-%m-%d %I:%M:%S %p").replace("AM", "ص").replace("PM", "م"),
                        "user_id": uid,
                        "name": uinfo_n.get("name") or uid,
                        "username": uinfo_n.get("username") or "بدون يوزر",
                        "channel": channel_info.get("title", channel_id),
                        "ch_id": channel_id,
                        "station": station_name,
                        "action": "بدأ البث",
                        "broadcast_type": "night",
                    })
...
async def _send_auto_backup(label):
    ...
    try:
        ...
        files = [
            ...
            ("/data/broadcast_log.json",           "📋 سجل البثوث"),
            ("/data/broadcast_stats.json",         "📊 إحصائيات البث"),
            ("/data/broadcast_start_times.json",   "⏰ أوقات بدء البث"),
        ]
...
def reset_broadcast_stats(channel_id, station_name="", user_id="", broadcast_type="manual"):
    """تصفير وقت البدء ومدة البث عند تغيير الإذاعة"""
...
def save_broadcast_state():
    ...
    for user_id, user_info in list(user_data.items()):
        channels = user_info.get("channels", {})
        for channel_id, channel_info in list(channels.items()):
            should = channel_info.get("should_broadcast", None)
            # ━━━ قناة موقفة يدوياً ━━━
            if channel_info.get("externally_stopped", False) or should is False:
                saved_broadcasts[channel_id] = {
                    "user_id": user_id,
                    "was_running": False,
                    "externally_stopped": True,
                    "should_broadcast": False,
                    "rtmps_url": channel_info.get("rtmps_url", ""),
                    "station": channel_info.get("station", ""),
                }
                continue
            # ━━━ قناة شغالة ━━━
            if "process" in channel_info or should is True:
                ...
                saved_broadcasts[channel_id] = {
                    ...
                    "schedule_enabled": channel_info.get("schedule_enabled", False),
                    "night_mode_enabled": channel_info.get("night_mode_enabled", False),
                }
    # احفظ في ملف
    try:
...
def restore_broadcasts():
    global saved_broadcasts
    # ━━━ امسح كل process قديم من user_data قبل أي حاجة ━━━
    for uid in list(user_data.keys()):
        for ch_id in list(user_data[uid].get("channels", {}).keys()):
    ...
    for channel_id, info in saved_broadcasts.items():
        try:
            uid = info["user_id"]
            ch_data = user_data.get(uid, {}).get("channels", {}).get(channel_id, {})

            # ━━━ المرجع الأساسي: should_broadcast ━━━
            # أولاً: جرب من user_data (الأحدث)
            if "should_broadcast" in ch_data:
                if not ch_data["should_broadcast"]:
                    logger.info(f"Skipping restore for {channel_id} — should_broadcast is False (user_data)")
                    continue
            # ثانياً: جرب من broadcast_state.json
            elif "should_broadcast" in info:
                if not info["should_broadcast"]:
                    logger.info(f"Skipping restore for {channel_id} — should_broadcast is False (state file)")
                    continue
            else:
                # fallback للمنطق القديم لو should_broadcast مش موجود (بيانات قديمة)
                if ch_data.get("externally_stopped", False):
                    logger.info(f"Skipping restore for {channel_id} — was manually stopped (legacy)")
                    continue
                if not info.get("was_running", False):
                    logger.info(f"Skipping restore for {channel_id} — was_running is False (legacy)")
                    continue

            # لو في جدول — شغّل الإذاعة المناسبة للوقت الحالي فوراً
            ch_has_schedule = ch_data.get("schedule_enabled", False) or ch_data.get("night_mode_enabled", False)
            if ch_has_schedule:
                current_time_r = time.strftime("%H:%M", time.gmtime(time.time() + 7200))
                today_r = time.strftime("%Y-%m-%d", time.gmtime(time.time() + 7200))
                restore_station = None
                restore_btype = None
                # جدول تلقائي
                if ch_data.get("schedule_enabled", False):
                    ch_schedule_r = ch_data.get("ch_schedule", AUTO_SCHEDULE)
                    ch_disabled_r = set(ch_data.get("schedule_disabled", []))
                    for t in reversed(sorted(ch_schedule_r.keys())):
                        if current_time_r >= t and t not in ch_disabled_r:
                            entry = ch_schedule_r[t]
                            if isinstance(entry, dict) and entry.get("url"):
                                restore_station = entry
                                restore_btype = "auto"
                                break
                    if not restore_station:
                        for t in reversed(sorted(ch_schedule_r.keys())):
                            if t not in ch_disabled_r:
                                entry = ch_schedule_r[t]
                                if isinstance(entry, dict) and entry.get("url"):
                                    restore_station = entry
                                    restore_btype = "auto"
                                    break
                # وضع ليلي (أولوية لو الجدول ماجابش حاجة)
                if not restore_station and ch_data.get("night_mode_enabled", False):
                    ch_night_r = ch_data.get("night_schedule", DEFAULT_NIGHT_SCHEDULE)
                    night_disabled_r = set(ch_data.get("night_disabled", []))
                    for t in reversed(sorted(ch_night_r.keys())):
                        if current_time_r >= t and t not in night_disabled_r:
                            entry = ch_night_r[t]
                            if isinstance(entry, dict) and entry.get("url"):
                                restore_station = entry
                                restore_btype = "night"
                ...
                if restore_station:
                    try:
                        rtmps_r = ch_data.get("rtmps_url", info.get("rtmps_url", ""))
                        if not rtmps_r:
                            logger.warning(f"restore schedule: no rtmps_url for channel {channel_id}")
                            continue
                        ffmpeg_cmd_r = build_ffmpeg_cmd(restore_station["url"], rtmps_r)
                        process_r = subprocess.Popen(ffmpeg_cmd_r, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                        uid_r = info["user_id"]
                        user_data[uid_r]["channels"][channel_id]["process"] = process_r.pid
                        user_data[uid_r]["channels"][channel_id]["station"] = restore_station["url"]
                        user_data[uid_r]["channels"][channel_id]["broadcast_type"] = restore_btype
                        broadcast_fail_count[channel_id] = 0
                        station_name_r = restore_station.get("name", "")
                        reset_broadcast_stats(channel_id, station_name_r, uid_r, restore_btype)
                        if channel_id not in broadcast_start_times:
                            broadcast_start_times[channel_id] = time.time()
                        logger.info(f"Restored schedule broadcast for channel {channel_id} → {station_name_r} ({restore_btype})")
                    except Exception as e:
                        logger.error(f"restore schedule start error for {channel_id}: {e}")
                else:
                    logger.info(f"Skipping restore for {channel_id} — has schedule but no station found")
                continue
            # تحقق إن الـ process القديم مش شغال فعلاً
            old_pid = ch_data.get("process")
            if old_pid and is_ffmpeg_running(old_pid):
                logger.info(f"Skipping restore for {channel_id} — ffmpeg already running (pid {old_pid})")
                continue
            ...
            user_data[uid]["channels"][channel_id]["process"] = process.pid
            user_data[uid]["channels"][channel_id].pop("externally_stopped", None)
            # استعادة حالة الجدول والوضع الليلي من الملف المحفوظ
            if "schedule_enabled" in info:
                user_data[uid]["channels"][channel_id]["schedule_enabled"] = info["schedule_enabled"]
            if "night_mode_enabled" in info:
                user_data[uid]["channels"][channel_id]["night_mode_enabled"] = info["night_mode_enabled"]
            broadcast_fail_count[channel_id] = 0
...
def build_bot_stats_text():
    ...
    auto_broadcasts = sum(
        1 for u in list(user_data.values())
        for ch in list(u.get("channels", {}).values())
        if ch.get("process") and is_ffmpeg_running(ch["process"]) and ch.get("broadcast_type") == "auto"
    )
    night_broadcasts = sum(
        1 for u in list(user_data.values())
        for ch in list(u.get("channels", {}).values())
        if ch.get("process") and is_ffmpeg_running(ch["process"]) and ch.get("broadcast_type") == "night"
    )
    ...
    return (
        ...
        f"📡 البثوث النشطة: {active_broadcasts}\n"
        f"  ├ 👤 يدوي: {active_broadcasts - auto_broadcasts - night_broadcasts}\n"
        f"  ├ 🤖 تلقائي: {auto_broadcasts}\n"
        f"  └ 🌙 ليلي: {night_broadcasts}\n"
        ...
        if (chat.type in (ChatType.CHANNEL, ChatType.SUPERGROUP, ChatType.GROUP) and
            ...
            if owner_uid and ch_id_str in user_data.get(owner_uid, {}).get("channels", {}):
                ...
                if "process" in ch_info:
                    ...
                    user_data[owner_uid]["channels"][ch_id_str]["should_broadcast"] = False
                    save_data()
...
def save_settings():
    ...
    try:
        settings = {
            ...
            "broadcast_log": broadcast_log[-200:] if len(broadcast_log) > 200 else broadcast_log,
        }
...
def load_settings():
    ...
    try:
        ...
        # REQUIRED_CHANNELS: لا تمسح القنوات الموجودة لو settings فاضية منها
        _rc_from_settings = s.get("REQUIRED_CHANNELS", [])
        if _rc_from_settings:
            REQUIRED_CHANNELS = _rc_from_settings
        elif not REQUIRED_CHANNELS:
            REQUIRED_CHANNELS = []
        ...
        global broadcast_log
        _saved_log = s.get("broadcast_log", [])
        if _saved_log:
            # دمج السجل المحفوظ مع الموجود في الذاكرة (تجنب التكرار)
            existing_times = {e.get("time","") for e in broadcast_log}
            for entry in _saved_log:
                if entry.get("time","") not in existing_times:
                    broadcast_log.append(entry)
                    existing_times.add(entry.get("time",""))
            broadcast_log = broadcast_log[-200:]
    ...
    try:
        ...
        for channel_id, channel_info in list(user_info.get("channels", {}).items()):
            ...
            try:
                ...
                broadcast_fail_count[channel_id] = 0
                # حافظ على broadcast_type الأصلي وحدّث وقت البدء
                btype_r = user_data[user_id]["channels"][channel_id].get("broadcast_type", "manual")
                sname_r = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), selected_station)
                reset_broadcast_stats(channel_id, sname_r, user_id, btype_r)
        ...
        for uid, user_info in list(user_data.items()):
            ...
            for channel_id, channel_info in list(channels.items()):
                ...
                try:
                    ...
                    now_str = egypt_strftime("%Y-%m-%d %I:%M:%S %p").replace("AM", "ص").replace("PM", "م")
                    reset_broadcast_stats(channel_id, station["name"], uid, "auto")
                    # إضافة للسجل
                    ...
                    auto_log_entry = {
                        ...
                        "username": uinfo.get("username") or "بدون يوزر",
                        "channel": channel_info.get("title", channel_id),
                        "ch_id": channel_id,
                    ...
                    broadcast_log.append(auto_log_entry)
                    save_broadcast_log_file()
                    save_data()
...
def _draw_ch_sched(ch_id, ch_info):
    ...
    for t, s in sorted(ch_schedule.items()):
        ...
        time_buttons.append([InlineKeyboardButton("⏹ إيقاف" if is_on else "▶️ تشغيل", callback_data=f"ch_sched_time_toggle|{ch_id}|{t}")])
    toggle_label = "❌ إيقاف الجدول" if ch_enabled else "✅ تفعيل الجدول"
    return (
        f"<blockquote>📅 جدول القناة\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
        InlineKeyboardMarkup(time_buttons + [[InlineKeyboardButton(toggle_label, callback_data=f"ch_sched_toggle|{ch_id}")]])
    )
...
def _draw_unight_sched(uid, ch_id, ch_info):
    ...
    for t, entry in sorted(night_schedule.items()):
        ...
        is_on = t not in night_disabled
        icon = "✅" if is_on else "❌"
        btn_label = "⏹ إيقاف" if is_on else "▶️ تشغيل"
        sched_buttons.append([InlineKeyboardButton(f"{icon} {to_ar(t)} | {sname}", callback_data="noop")])
        sched_buttons.append([InlineKeyboardButton(btn_label, callback_data=f"unight_toggle_entry|{uid}|{ch_id}|{t}")])
    night_toggle_label = "❌ إيقاف الوضع الليلي" if ch_night else "✅ تفعيل الوضع الليلي"
    return (
        f"<blockquote>🌙 الوضع الليلي\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
        InlineKeyboardMarkup(sched_buttons + [[InlineKeyboardButton(night_toggle_label, callback_data=f"unight_toggle|{uid}|{ch_id}")]])
    )
                    ...
                    if fail_count >= broadcast_fail_threshold:
                        ...
                        try:
                            asyncio.run_coroutine_threadsafe(
                                app.send_message(
                                    ...
                                    f"• يمكنك بدء البث مجدداً من البوت عند الحاجة.</blockquote>"
                                ), _bot_loop
                            )
                        except Exception:
                            pass
                        ...
                        try:
                            ...
                            if is_ffmpeg_running(process.pid):
                                ...
                                restarted_ok = True
                                # حدّث stats بدون تغيير broadcast_type
                                btype_w = channel_info.get("broadcast_type", "manual")
                                sname_w = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), selected_station)
                                reset_broadcast_stats(channel_id, sname_w, user_id, btype_w)
                        ...
                        _fail_started.pop(channel_id, None)
                        if restarted_ok:
                            # cooldown: مش نبعت رسالة أكتر من مرة كل 30 دقيقة لنفس القناة
                            last_notify = _notify_cooldown.get(channel_id, 0)
                            if time.time() - last_notify >= 1800:
                                ...
                                try:
                                    ...
                                    asyncio.run_coroutine_threadsafe(
                                        app.send_message(
                                            ...
                                            reply_markup=InlineKeyboardMarkup([
                                                [InlineKeyboardButton("⏹ إيقاف البث", callback_data=f"stop_ch_confirm_{channel_id}")]
                                            ])
                                        ), _bot_loop
                                    )
                                except Exception:
...
async def _show_channels(client, message, filter_type="all", search_query=None):
    ...
    try:
        ...
        for uid, user_info in sorted_users:
            ...
            try:
                owner_name = user_info.get("name") or uid
                owner_username = f"@{user_info['username']}" if user_info.get("username") else None
                owner_link = get_user_link(uid, user_info.get("username") or None)
            except Exception as e:
...
async def _show_users(client, message, filter_type="all", search_query=None):
    ...
    for uid, user_info in sorted_users:
        ...
        if filter_type == "banned" and not is_banned:
            continue
        try:
            name = user_info.get("name") or uid
            username = f"@{user_info['username']}" if user_info.get("username") else "بدون يوزر"
            user_link = get_user_link(uid, user_info.get("username") or None)
        except Exception as e:
...
# ===== ttype encode/decode لتقليل حجم callback_data =====
_TTYPE_ENC = {
    "unight_sched_time": "ust",
    "ch_sched_add_time":  "csat",
    "ch_sched_time":      "cst",
    "unight_sched":       "us",
    "unight_time":        "ut",
    "daily_report":       "dr",
...
_TTYPE_DEC = {v: k for k, v in _TTYPE_ENC.items()}

def _enc_ttype(t): return _TTYPE_ENC.get(t, t)
def _dec_ttype(t): return _TTYPE_DEC.get(t, t)
# =========================================================
def build_time_picker_kb(h, m, ttype="sched", old_time=""):
    ...
    h12 = h % 12 or 12
    period = "🌅 صباحاً" if h < 12 else "🌙 مساءً"
    # نستخدم | كـ separator عشان ttype وold_time قد يحتووا على _
    base = f"{h}|{m}|{_enc_ttype(ttype)}|{old_time}"
    buttons = [
        # الساعة
        [InlineKeyboardButton("─── الساعة ───", callback_data="noop")],
        [
            InlineKeyboardButton("◀️", callback_data=f"tp_h_dec|{base}"),
            InlineKeyboardButton(f"  {h12}  ", callback_data="noop"),
            InlineKeyboardButton("▶️", callback_data=f"tp_h_inc|{base}"),
        ],
        # الدقيقة
        [InlineKeyboardButton("─── الدقيقة ───", callback_data="noop")],
        [
            InlineKeyboardButton("◀️", callback_data=f"tp_m_dec|{base}"),
            InlineKeyboardButton(f"  {m:02d}  ", callback_data="noop"),
            InlineKeyboardButton("▶️", callback_data=f"tp_m_inc|{base}"),
        ],
        # صباح/مساء
        [InlineKeyboardButton(period, callback_data=f"tp_period|{base}")],
        # تأكيد
        [InlineKeyboardButton(f"✅ تأكيد — {h12}:{m:02d} {period}", callback_data=f"tp_confirm|{base}")],
    ]
    if ttype == "bsched":
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
    ...
    elif data.startswith("stop_ch_confirm_"):
        # خطوة التأكيد — يعرض رسالة تأكيد قبل إيقاف البث
        await query.answer()
        ch_id = data[len("stop_ch_confirm_"):]
        uid = user_id
        for uid_key, uinfo in user_data.items():
            if ch_id in uinfo.get("channels", {}):
                uid = uid_key
                break
        ...
        if not channel_info:
            ...
            return
        ch_title = channel_info.get("title", ch_id)
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        await query.edit_message_text(
            f"<blockquote>⚠️ تأكيد إيقاف البث\n\n"
            f"📢 {ch_title}\n\n"
            f"هل أنت متأكد أنك تريد إيقاف البث؟</blockquote>",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ نعم، أوقف", callback_data=f"stop_ch_{ch_id}"),
                    InlineKeyboardButton("❌ إلغاء", callback_data="stop_confirm_cancel"),
                ]
            ])
        )

    elif data == "stop_confirm_cancel":
        await query.answer("تم الإلغاء ✅", show_alert=False)
        _cancel_uid = user_id
        _cancel_kb = admin_keyboard(_cancel_uid) if is_admin(int(_cancel_uid)) else user_keyboard(_cancel_uid)
        try:
            await query.message.delete()
        except Exception:
            pass
        await app.send_message(
            int(_cancel_uid),
            "<blockquote>❌ تم إلغاء العملية\n\nعُدت للقائمة الرئيسية ✅</blockquote>",
        ...
        uid = user_id
        # ابحث عن صاحب القناة (قد يكون أدمن يوقف قناة مستخدم)
        for uid_key, uinfo in user_data.items():
            if ch_id in uinfo.get("channels", {}):
                uid = uid_key
                break
        ...
        if not channel_info:
            await query.answer("❌ القناة غير موجودة", show_alert=True)
            return
        btype = channel_info.get("broadcast_type", "manual")
        was_auto = btype == "auto"
        was_night = btype == "night"
        ch_title = channel_info.get("title", ch_id)
        try:
            if "process" in channel_info:
                pid = channel_info["process"]
                if is_ffmpeg_running(pid):
                    subprocess.run(["kill", "-9", str(pid)], timeout=5, check=True)
                user_data[uid]["channels"][ch_id].pop("process", None)
                user_data[uid]["channels"][ch_id].pop("broadcast_type", None)
            # أوقف الجدول والوضع الليلي دايماً عند الإيقاف اليدوي — بغض النظر عن وجود process
            user_data[uid]["channels"][ch_id]["schedule_enabled"] = False
            user_data[uid]["channels"][ch_id]["night_mode_enabled"] = False
            user_data[uid]["channels"][ch_id]["externally_stopped"] = True
            user_data[uid]["channels"][ch_id]["should_broadcast"] = False
            # احفظ القناة كموقفة في broadcast_state عشان بعد restart ميشغلهاش
            saved_broadcasts[ch_id] = {
                "user_id": uid,
                "was_running": False,
                "rtmps_url": user_data[uid]["channels"][ch_id].get("rtmps_url", ""),
                "station": user_data[uid]["channels"][ch_id].get("station", ""),
            }
            save_broadcast_state()
            save_data()
        except Exception as e:
        ...
        _uid_int = int(uid)
        _main_kb = admin_keyboard(uid) if is_admin(_uid_int) else user_keyboard(uid)
        if was_auto:
            await query.edit_message_text(
                f"<blockquote>⏹ تم إيقاف الجدول التلقائي والإذاعة\n\n"
                f"📢 {ch_title}\n"
                f"📅 {egypt_strftime()}\n\n"
                f"🎙 متى ما أردت العودة للبث، اضغط <b>بدء البث</b> واختر إذاعتك.</blockquote>"
            )
            await app.send_message(_uid_int, "🏠 اللوحة الرئيسية", reply_markup=_main_kb)
        elif was_night:
            await query.edit_message_text(
                f"<blockquote>⏹ تم إيقاف الوضع الليلي والإذاعة\n\n"
                f"📢 {ch_title}\n"
                f"📅 {egypt_strftime()}\n\n"
                f"🎙 متى ما أردت العودة للبث، اضغط <b>بدء البث</b> واختر إذاعتك.</blockquote>"
            )
        ...
        if len(active_channels) == 1:
            ...
            for ch_id, ch_info in active_channels.items():
                buttons.append([InlineKeyboardButton(
                    ...
                    callback_data=f"stop_ch_confirm_{ch_id}"
                )])
            buttons.append([InlineKeyboardButton("⏹ إيقاف الكل", callback_data="stop_all_confirm")])
            await query.edit_message_text(
        ...
        buttons = [
            ...
            [InlineKeyboardButton("⏹ إيقاف جميع بثوث المستخدمين", callback_data="stop_users_all_confirm")],
        ]
        await query.edit_message_text(
        ...
        for uid, uinfo in user_data.items():
            ...
            for ch_id, ch_info in uinfo.get("channels", {}).items():
                if "process" in ch_info:
                    ...
                    buttons.append([InlineKeyboardButton(
                        f"⏹ {ch_title} — {uname}",
                        callback_data=f"stop_user_ch_confirm_{uid}_{ch_id}"
                    )])
        if not buttons:
    ...
    elif data == "stop_users_all_confirm":
        await query.answer()
        uid_int = int(user_id)
        if uid_int != OWNER_ID and not has_perm(uid_int, "stop_users_broadcast"):
            await query.answer("❌ ليس لديك صلاحية", show_alert=True)
            return
        active = sum(
            1 for uid, uinfo in user_data.items()
            if uid != user_id
            for ch in uinfo.get("channels", {}).values()
            if "process" in ch
        )
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        await query.edit_message_text(
            f"<blockquote>⚠️ تأكيد إيقاف جميع بثوث المستخدمين\n\n"
            f"📢 البثوث النشطة: {active}\n\n"
            f"هل أنت متأكد؟</blockquote>",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ نعم، أوقف الكل", callback_data="stop_users_all"),
                    InlineKeyboardButton("❌ إلغاء", callback_data="stop_confirm_cancel"),
                ]
            ])
        )
        ...
        for uid, uinfo in user_data.items():
            if uid == user_id:
                continue
            for ch_id, ch_info in list(uinfo.get("channels", {}).items()):
                if "process" in ch_info or ch_info.get("schedule_enabled") or ch_info.get("night_mode_enabled"):
                    try:
                        if "process" in ch_info:
    ...
    elif data.startswith("stop_user_ch_confirm_"):
        await query.answer()
        uid_int = int(user_id)
        if uid_int != OWNER_ID and not has_perm(uid_int, "stop_users_broadcast"):
            await query.answer("❌ ليس لديك صلاحية", show_alert=True)
            return
        rest = data[len("stop_user_ch_confirm_"):]
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ نعم، أوقف", callback_data=f"stop_user_ch_{target_uid}_{ch_id}"),
                    InlineKeyboardButton("❌ إلغاء", callback_data="stop_confirm_cancel"),
                ]
            ])
        )
        ...
        try:
            ...
            user_data[target_uid]["channels"][ch_id].pop("broadcast_type", None)
            user_data[target_uid]["channels"][ch_id]["externally_stopped"] = True
            user_data[target_uid]["channels"][ch_id]["should_broadcast"] = False
            user_data[target_uid]["channels"][ch_id]["schedule_enabled"] = False
            user_data[target_uid]["channels"][ch_id]["night_mode_enabled"] = False
            save_data()
        except Exception as e:
        ...
        await query.answer()
        uid = user_id
        active = sum(1 for ch in user_data.get(uid, {}).get("channels", {}).values() if "process" in ch)
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        await query.edit_message_text(
            f"<blockquote>⚠️ تأكيد إيقاف جميع البثوث\n\n"
            f"📢 القنوات النشطة: {active}\n\n"
            f"هل أنت متأكد؟</blockquote>",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ نعم، أوقف الكل", callback_data="stop_all"),
                    InlineKeyboardButton("❌ إلغاء", callback_data="stop_confirm_cancel"),
                ]
            ])
        )
        ...
        save_broadcast_state()
        save_data()
        _uid_all_int = int(uid)
        _main_kb_all = admin_keyboard(uid) if is_admin(_uid_all_int) else user_keyboard(uid)
        await query.edit_message_text(
            f"<blockquote>⏹ تم إيقاف جميع البثوث\n\n"
            f"📢 القنوات المتوقفة: {stopped}\n"
            f"📅 {egypt_strftime()}</blockquote>",
            reply_markup=_main_kb_all
        )
        ...
        try:
            ...
            btype_rn = channel_info.get("broadcast_type", "manual")
            sname_rn = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), selected_station)
            reset_broadcast_stats(ch_id, sname_rn, uid, btype_rn)
            save_data()
            await query.edit_message_text(
    ...
    elif data.startswith("tp_h_dec|") or data.startswith("tp_h_inc|"):
        await query.answer()
        try:
            # format: tp_h_dec|h|m|ttype|old_time
            rest = data.split("|", 1)[1]
            parts = rest.split("|", 3)
            h, m = int(parts[0]), int(parts[1])
            ttype = _dec_ttype(parts[2]) if len(parts) > 2 else "sched"
            old_time = parts[3] if len(parts) > 3 else ""
            action = "inc" if "inc" in data.split("|")[0] else "dec"
        except (IndexError, ValueError):
            await query.answer("❌ خطأ في البيانات", show_alert=True)
            return
        h = (h + 1) % 24 if action == "inc" else (h - 1) % 24
        await query.edit_message_reply_markup(build_time_picker_kb(h, m, ttype, old_time))

    elif data.startswith("tp_m_dec|") or data.startswith("tp_m_inc|"):
        await query.answer()
        try:
            rest = data.split("|", 1)[1]
            parts = rest.split("|", 3)
            ...
            await query.answer("❌ خطأ في البيانات", show_alert=True)
            return
        m = (m + 5) % 60 if action == "inc" else (m - 5) % 60
        await query.edit_message_reply_markup(build_time_picker_kb(h, m, ttype, old_time))

    elif data.startswith("tp_period|"):
        await query.answer()
        try:
            rest = data.split("|", 1)[1]
            parts = rest.split("|", 3)
            ...
            await query.answer("❌ خطأ في البيانات", show_alert=True)
            return
        h = h + 12 if h < 12 else h - 12
        await query.edit_message_reply_markup(build_time_picker_kb(h, m, ttype, old_time))

    elif data.startswith("tp_confirm|"):
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        try:
            rest = data.split("|", 1)[1]
        ...
        elif ttype == "unight_time":
            # old_time = uid|ch_id|old_t  — تعديل وقت موعد في الجدول الليلي
            parts = old_time.split("|", 2)
            _uid = parts[0]
        ...
        def to_ar_time(t):
            try:
                ...
                return t
        # بناء أزرار الجدول
        _night_disabled = set(ch_info.get("night_disabled", []))
        sched_buttons = []
        for t, entry in sorted(night_schedule.items()):
            if isinstance(entry, dict):
            ...
            _is_on = t not in _night_disabled
            _icon = "✅" if _is_on else "❌"
            _btn_label = "⏹ إيقاف" if _is_on else "▶️ تشغيل"
            sched_buttons.append([
                InlineKeyboardButton(f"{_icon} {to_ar_time(t)} | {sname}", callback_data="noop")
            ])
            sched_buttons.append([
                InlineKeyboardButton(_btn_label, callback_data=f"unight_toggle_entry|{uid}|{ch_id}|{t}"),
            ])
        night_toggle_label = "❌ إيقاف الوضع الليلي" if ch_night else "✅ تفعيل الوضع الليلي"
        await query.edit_message_text(
            f"<blockquote>🌙 الوضع الليلي\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                sched_buttons + [
                    [InlineKeyboardButton(night_toggle_label, callback_data=f"unight_toggle|{uid}|{ch_id}")],
                ]
            )
        )
    ...
    elif data.startswith("unight_scat|"):
        # format: unight_scat|uid|ch_id|cat_index
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        cat_raw = parts[3] if len(parts) > 3 else ""
        cat = CAT_BY_INDEX.get(cat_raw, cat_raw)
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        ...
        for i, sid in enumerate(station_ids, 1):
            name = ST_TIMO[sid]["name"].replace("إذاعة ", "").replace("اذاعة ", "").replace("قناة ", "").strip()
            row.append(InlineKeyboardButton(f"{name}", callback_data=f"unight_sched_pick|{uid}|{ch_id}|{sid}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
    ...
    elif data.startswith("unight_add_sched|"):
        # format: unight_add_sched|uid|ch_id
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        await query.answer()
        ...
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_scat|{uid}|{ch_id}|{CAT_INDEX.get(cat_name, cat_name)}")])
        await query.edit_message_text(
            "<blockquote>➕ إضافة موعد ليلي\n\nاختر القسم:</blockquote>",
            reply_markup=InlineKeyboardMarkup(buttons)
        )

    elif data.startswith("unight_sched_cat|"):
        # format: unight_sched_cat|uid|ch_id|cat_index
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        cat_raw = parts[3] if len(parts) > 3 else ""
    ...
    elif data.startswith("unight_sched_pick|"):
        # format: unight_sched_pick|uid|ch_id|sid
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        sid = parts[3] if len(parts) > 3 else "1"
        await query.answer()
        station_name = ST_TIMO.get(sid, {}).get("name", sid)
        h, m = 23, 0
        await query.edit_message_text(
            f"<blockquote>➕ {station_name}\n\nاختر وقت التشغيل:</blockquote>",
            reply_markup=build_time_picker_kb(h, m, "unight_sched", f"{uid}|{ch_id}|{sid}")
        )

    elif data.startswith("unight_del_sched|"):
        # format: unight_del_sched|uid|ch_id
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        await query.answer()
        ...
        for t, sid in sorted(night_schedule.items()):
            sname = ST_TIMO.get(sid, {}).get("name", sid).replace("إذاعة ", "").strip() if isinstance(sid, str) else sid.get("name", "").replace("إذاعة ", "").strip()
            buttons.append([InlineKeyboardButton(f"🗑 {to_ar_time(t)} ← {sname}", callback_data=f"unight_del_entry|{uid}|{ch_id}|{t}")])
        await query.edit_message_text(
            "<blockquote>🗑 اختر الموعد لحذفه:</blockquote>",
            reply_markup=InlineKeyboardMarkup(buttons)
        )

    elif data.startswith("unight_edit_time|"):
        # format: unight_edit_time|uid|ch_id|HH:MM
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        old_t = parts[3] if len(parts) > 3 else ""
        await query.answer()
        h, m = map(int, old_t.split(":")) if ":" in old_t else (23, 0)
        await query.edit_message_text(
            f"<blockquote>⏰ تعديل وقت الموعد ({old_t})\n\nاستخدم الأزرار:</blockquote>",
            reply_markup=build_time_picker_kb(h, m, "unight_sched_time", f"{uid}|{ch_id}|{old_t}")
        )

    elif data.startswith("unight_edit_station|"):
        # format: unight_edit_station|uid|ch_id|HH:MM
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        ...
        buttons = []
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_edit_cat|{uid}|{ch_id}|{t}|{CAT_INDEX.get(cat_name, cat_name)}")])
        await query.edit_message_text(
            f"<blockquote>🎙 تغيير محطة الساعة {t}\n\nاختر القسم:</blockquote>",
            reply_markup=InlineKeyboardMarkup(buttons)
        )

    elif data.startswith("unight_edit_cat|"):
        # format: unight_edit_cat|uid|ch_id|HH:MM|cat_index
        parts = data.split("|", 4)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        t = parts[3] if len(parts) > 3 else ""
        cat_raw = parts[4] if len(parts) > 4 else ""
        cat = CAT_BY_INDEX.get(cat_raw, cat_raw)
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        station_ids = ST_CATEGORIES.get(cat, [])
        ...
        for i, sid in enumerate(station_ids, 1):
            name = ST_TIMO[sid]["name"].replace("إذاعة ", "").replace("اذاعة ", "").replace("قناة ", "").strip()
            row.append(InlineKeyboardButton(f"{name}", callback_data=f"unight_edit_pick|{uid}|{ch_id}|{t}|{sid}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
    ...
    elif data.startswith("unight_edit_pick|"):
        # format: unight_edit_pick|uid|ch_id|HH:MM|sid
        parts = data.split("|", 4)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        t = parts[3] if len(parts) > 3 else ""
    ...
    elif data.startswith("unight_del_entry|"):
        # format: unight_del_entry|uid|ch_id|HH:MM
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        t = parts[3] if len(parts) > 3 else ""
        ...
        for sid in u_night_stations:
            if sid in ST_TIMO:
                name = ST_TIMO[sid]["name"].replace("إذاعة ", "").replace("اذاعة ", "").strip()
                buttons.append([InlineKeyboardButton(f"🗑 {name}", callback_data=f"unight_del_station|{uid}|{sid}")])
        buttons.append([InlineKeyboardButton("➕ إضافة محطة ليلية", callback_data=f"unight_add_station|{uid}")])
    ...
    elif data.startswith("unight_add_station|"):
        # format: unight_add_station|uid
        uid = data.split("|", 1)[1]
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        buttons = []
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_add_cat|{uid}|{CAT_INDEX.get(cat_name, cat_name)}")])
    ...
    elif data.startswith("unight_add_cat|"):
        # format: unight_add_cat|uid|cat_index
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
        cat_raw = parts[2] if len(parts) > 2 else ""
        cat = CAT_BY_INDEX.get(cat_raw, cat_raw)
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        station_ids = ST_CATEGORIES.get(cat, [])
        ...
        for i, sid in enumerate(station_ids, 1):
            ...
            row.append(InlineKeyboardButton(f"{already}{name}", callback_data=f"unight_add_pick|{uid}|{sid}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
            buttons.append(row)
        buttons.append([InlineKeyboardButton("🔙 رجوع", callback_data=f"unight_add_station|{uid}")])
        await query.edit_message_text(
            f"<blockquote>➕ {cat}\n\nاختر المحطة (✅ = موجودة بالفعل):</blockquote>",
            reply_markup=InlineKeyboardMarkup(buttons)
        )

    elif data.startswith("unight_add_pick|"):
        # format: unight_add_pick|uid|sid
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
    ...
    elif data.startswith("unight_del_station|"):
        # format: unight_del_station|uid|sid
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
        ...
        for s in u_night_stations:
            if s in ST_TIMO:
                name = ST_TIMO[s]["name"].replace("إذاعة ", "").replace("اذاعة ", "").strip()
                buttons.append([InlineKeyboardButton(f"🗑 {name}", callback_data=f"unight_del_station|{uid}|{s}")])
        buttons.append([InlineKeyboardButton("➕ إضافة محطة ليلية", callback_data=f"unight_add_station|{uid}")])
        station_name = ST_TIMO.get(sid, {}).get("name", sid)
        await query.edit_message_text(
            f"<blockquote>✅ تم حذف {station_name}\n\n"
    ...
    elif data.startswith("unight_toggle_entry|"):
        # إيقاف/تشغيل محطة واحدة في الوضع الليلي
        await query.answer()
        # format: unight_toggle_entry|uid|ch_id|HH:MM
        parts = data.split("|", 3)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        t = parts[3] if len(parts) > 3 else ""
    ...
    elif data.startswith("unight_toggle|"):
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        # format: unight_toggle|uid|ch_id
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
        ch_id = parts[2] if len(parts) > 2 else ""
        if uid not in user_data or ch_id not in user_data[uid].get("channels", {}):
            ...
            return
        ch_info = user_data[uid]["channels"][ch_id]
        current = ch_info.get("night_mode_enabled", False)
        ch_title = ch_info.get("title", ch_id)
        # لو في بث يدوي شغال فقط — اسأل تأكيد (الجدول التلقائي يشتغل مع الوضع الليلي بدون سؤال)
        if not current:
            pid = ch_info.get("process")
            has_active = pid and is_ffmpeg_running(pid)
            btype = ch_info.get("broadcast_type", "manual")
            if has_active and btype == "manual":
                current_station_url = ch_info.get("station", "")
                current_station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == current_station_url), "البث الحالي")
                await query.edit_message_text(
                    f"<blockquote>⚠️ يوجد بث يدوي شغّال على قناة\n📢 {ch_title}\n"
                    f"🎙 الإذاعة الحالية: {current_station_name}\n\n"
                    f"هل تريد إيقافه وتفعيل الوضع الليلي؟</blockquote>",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ نعم، فعّل الوضع الليلي", callback_data=f"confirm_night|{uid}|{ch_id}")
                    ]])
                )
                return
        ...
        if not current:
            user_data[uid]["channels"][ch_id].pop("externally_stopped", None)
            user_data[uid]["channels"][ch_id]["should_broadcast"] = True
            # ━━━ شغّل فوراً الإذاعة الليلية المناسبة للوقت الحالي ━━━
            # بس لو الوقت الحالي فعلاً في نطاق الجدول الليلي
            ch_info_now = user_data[uid]["channels"][ch_id]
            current_time_n = time.strftime("%H:%M", time.gmtime(time.time() + 7200))
            ch_night_now = ch_info_now.get("night_schedule", DEFAULT_NIGHT_SCHEDULE)
            night_disabled_now = set(ch_info_now.get("night_disabled", []))
            # جيب أول وآخر وقت في الجدول الليلي
            active_times = [t for t in ch_night_now.keys() if t not in night_disabled_now]
            night_start = min(active_times) if active_times else None
            night_end = max(active_times) if active_times else None
            in_night_range = False
            if night_start and night_end:
                # الوضع الليلي بيمتد على منتصف الليل (مثلاً 23:00 لـ 05:00)
                if night_start > night_end:
                    in_night_range = current_time_n >= night_start or current_time_n <= night_end
                else:
                    in_night_range = night_start <= current_time_n <= night_end
            current_night_station = None
            if in_night_range:
                for t in reversed(sorted(ch_night_now.keys())):
                    if current_time_n >= t and t not in night_disabled_now:
                        entry = ch_night_now[t]
                        if isinstance(entry, dict) and entry.get("url"):
                            current_night_station = entry
                            break
            if current_night_station:
                try:
                    pid = ch_info_now.get("process")
                    if pid and is_ffmpeg_running(pid):
                        subprocess.run(["kill", "-9", str(pid)], timeout=5, check=True)
                    ffmpeg_cmd_n = build_ffmpeg_cmd(current_night_station["url"], ch_info_now["rtmps_url"])
                    process_n = subprocess.Popen(ffmpeg_cmd_n, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                    user_data[uid]["channels"][ch_id]["process"] = process_n.pid
                    user_data[uid]["channels"][ch_id]["station"] = current_night_station["url"]
                    user_data[uid]["channels"][ch_id]["broadcast_type"] = "night"
                    reset_broadcast_stats(ch_id, current_night_station["name"], uid, "night")
                    logger.info(f"Night mode enabled: immediate start {current_night_station['name']} for channel {ch_id}")
                except Exception as e:
                    logger.error(f"unight_toggle immediate start error: {e}")
        else:
            user_data[uid]["channels"][ch_id]["should_broadcast"] = False
        save_data()
        ch_info = user_data[uid]["channels"][ch_id]
        # لو كان إيقاف الوضع الليلي — رجّع للقائمة الرئيسية
        if current:
            _uid_night_int = int(uid)
            _main_kb_night = admin_keyboard(uid) if is_admin(_uid_night_int) else user_keyboard(uid)
            try:
                await query.message.delete()
            except Exception:
                pass
            await app.send_message(
                _uid_night_int,
                f"<blockquote>⏹ تم إيقاف الوضع الليلي\n\n"
                f"📢 {ch_title}\n"
                f"📅 {egypt_strftime()}\n\n"
                f"🎙 متى ما أردت العودة للبث، اضغط <b>بدء البث</b> واختر إذاعتك.</blockquote>",
                reply_markup=_main_kb_night
            )
        else:
            txt, kb = _draw_unight_sched(uid, ch_id, ch_info)
            await query.edit_message_text(txt, reply_markup=kb)

    elif data.startswith("confirm_night|"):
        # تأكيد إيقاف البث وتفعيل الوضع الليلي
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        parts = data.split("|", 2)
        uid = parts[1] if len(parts) > 1 else ""
        ...
        try:
            ...
            logger.error(f"confirm_night kill error: {e}")
        # فعّل الوضع الليلي
        user_data[uid]["channels"][ch_id]["night_mode_enabled"] = True
        save_data()
        ch_info = user_data[uid]["channels"][ch_id]
        txt, kb = _draw_unight_sched(uid, ch_id, ch_info)
        await query.edit_message_text(
            f"<blockquote>✅ تم تفعيل الوضع الليلي\n📢 {ch_info.get('title', ch_id)}</blockquote>"
        )
        ...
        try:
            ...
            pass
        start_time_display = broadcast_stats.get(channel_id, {}).get("start_time", "—") if is_ch_active else "—"
        btype_ch = broadcast_stats.get(channel_id, {}).get("broadcast_type", info.get("broadcast_type", "manual")) if is_ch_active else "—"
        btype_ch_label = "🤖 تلقائي" if btype_ch == "auto" else "🌙 ليلي" if btype_ch == "night" else "👤 يدوي" if btype_ch != "—" else "—"
        text_msg = (
            f"<blockquote>"
            f"📢 {info['title']}\n"
            f"👥 الأعضاء: {members}\n"
            f"📡 البث: {status}\n"
            f"🎙 المحطة: {station_name}\n"
            f"🔀 نوع البث: {btype_ch_label}\n"
            f"⏰ وقت البدء: {start_time_display}\n"
            f"⏱ مدة البث: {duration}\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"🆔 المعرف: {channel_id}"
            f"</blockquote>"
        ...
        await query.edit_message_text(
            f"<blockquote>⚠️ تأكيد حذف القناة\n\n📢 {ch_title}\n\nهل أنت متأكد أنك تريد حذف القناة؟</blockquote>",
            reply_markup=InlineKeyboardMarkup([
        ...
        for t, s in sorted(ch_schedule.items()):
            ...
            time_buttons.append([
                InlineKeyboardButton("⏹ إيقاف" if is_on else "▶️ تشغيل", callback_data=f"ch_sched_time_toggle|{ch_id}|{t}"),
            ])
        toggle_label = "❌ إيقاف الجدول" if ch_enabled else "✅ تفعيل الجدول"
        await query.edit_message_text(
            f"<blockquote>📅 جدول القناة\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                time_buttons + [
                    [InlineKeyboardButton(toggle_label, callback_data=f"ch_sched_toggle|{ch_id}")],
                ]
            )
        )
    ...
    elif data.startswith("ch_sched_action|"):
        # format: ch_sched_action|action|ch_id|HH:MM
        parts = data.split("|", 3)
        ...
        if action == "edit_time":
            ...
            await query.edit_message_text(
                ...
                reply_markup=build_time_picker_kb(h, m, "ch_sched_time", f"{owner_id}|{ch_id}|{t}")
            )
        elif action == "edit_station":
            from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
            buttons = []
            for cat_name in ST_CATEGORIES.keys():
                buttons.append([InlineKeyboardButton(cat_name, callback_data=f"ch_sched_st_cat|{ch_id}|{t}|{CAT_INDEX.get(cat_name, cat_name)}")])
            await query.edit_message_text(
                f"<blockquote>🎙 تغيير محطة الساعة {t}\n\nاختر القسم:</blockquote>",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
        elif action == "toggle":
            if "ch_schedule" not in user_data.get(owner_id, {}).get("channels", {}).get(ch_id, {}):
                user_data[owner_id]["channels"][ch_id]["ch_schedule"] = dict(AUTO_SCHEDULE)
            ch_disabled = set(user_data[owner_id]["channels"][ch_id].get("schedule_disabled", []))
            if t in ch_disabled:
                ch_disabled.discard(t)
            else:
                ch_disabled.add(t)
            user_data[owner_id]["channels"][ch_id]["schedule_disabled"] = list(ch_disabled)
            save_data()
            await query.answer("✅ تم تغيير الحالة", show_alert=True)
            ch_info = user_data.get(owner_id, {}).get("channels", {}).get(ch_id, {})
            txt, kb = _draw_ch_sched(ch_id, ch_info)
            await query.edit_message_text(txt, reply_markup=kb)
        elif action == "delete":
            if "ch_schedule" not in user_data.get(owner_id, {}).get("channels", {}).get(ch_id, {}):
                user_data[owner_id]["channels"][ch_id]["ch_schedule"] = dict(AUTO_SCHEDULE)
            ch_sched = user_data[owner_id]["channels"][ch_id]["ch_schedule"]
            if t in ch_sched:
                del ch_sched[t]
                save_data()
    ...
    elif data.startswith("unight_action|"):
        # format: unight_action|action|uid|ch_id|HH:MM
        parts = data.split("|", 4)
        action = parts[1] if len(parts) > 1 else ""
        uid = parts[2] if len(parts) > 2 else ""
        ch_id = parts[3] if len(parts) > 3 else ""
        t = parts[4] if len(parts) > 4 else ""
        await query.answer()
        if action == "edit_time":
            h, m = map(int, t.split(":")) if ":" in t else (23, 0)
            await query.edit_message_text(
                f"<blockquote>⏰ تعديل وقت المحطة الليلية ({t})\n\nاستخدم الأزرار:</blockquote>",
                reply_markup=build_time_picker_kb(h, m, "unight_time", f"{uid}|{ch_id}|{t}")
            )
        elif action == "edit_station":
            from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
            buttons = []
    ...
    elif data.startswith("ch_sched_edit_time|"):
        # format: ch_sched_edit_time|ch_id|HH:MM
        parts = data.split("|", 2)
        ch_id = parts[1] if len(parts) > 1 else ""
        ...
        await query.edit_message_text(
            f"<blockquote>⏰ تعديل وقت الموعد ({old_t})\n\nاستخدم الأزرار:</blockquote>",
            reply_markup=build_time_picker_kb(h, m, "ch_sched_time", f"{user_id}|{ch_id}|{old_t}")
        )

    elif data.startswith("ch_sched_edit_station|"):
        # format: ch_sched_edit_station|ch_id|HH:MM
        parts = data.split("|", 2)
        ch_id = parts[1] if len(parts) > 1 else ""
    ...
    elif data.startswith("ch_sched_st_cat|"):
        # format: ch_sched_st_cat|ch_id|HH:MM|cat_index
        parts = data.split("|", 3)
        ch_id = parts[1] if len(parts) > 1 else ""
        t = parts[2] if len(parts) > 2 else ""
        cat_raw = parts[3] if len(parts) > 3 else ""
        ...
        for i, sid in enumerate(station_ids, 1):
            name = ST_TIMO[sid]["name"].replace("إذاعة ", "").replace("اذاعة ", "").strip()
            row.append(InlineKeyboardButton(f"{name}", callback_data=f"ch_sched_st_pick|{ch_id}|{t}|{sid}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
    ...
    elif data.startswith("ch_sched_st_pick|"):
        # format: ch_sched_st_pick|ch_id|HH:MM|sid
        parts = data.split("|", 3)
        ch_id = parts[1] if len(parts) > 1 else ""
        t = parts[2] if len(parts) > 2 else ""
        ...
        if "ch_schedule" not in user_data.get(owner_id, {}).get("channels", {}).get(ch_id, {}):
            user_data[owner_id]["channels"][ch_id]["ch_schedule"] = dict(AUTO_SCHEDULE)
        user_data[owner_id]["channels"][ch_id]["ch_schedule"][t] = {
            "name": ST_TIMO[sid]["name"],
            "url": ST_TIMO[sid]["url"]
        }
        save_data()
    ...
    elif data.startswith("ch_sched_del|"):
        # format: ch_sched_del|ch_id|HH:MM
        parts = data.split("|", 2)
        ch_id = parts[1] if len(parts) > 1 else ""
        t = parts[2] if len(parts) > 2 else ""
        await query.answer()
    ...
    elif data.startswith("ch_sched_add|"):
        # format: ch_sched_add|ch_id
        ch_id = data.split("|", 1)[1]
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        buttons = []
        for cat_name in ST_CATEGORIES.keys():
            buttons.append([InlineKeyboardButton(cat_name, callback_data=f"ch_sched_add_cat|{ch_id}|{CAT_INDEX.get(cat_name, cat_name)}")])
        await query.edit_message_text(
            "<blockquote>➕ إضافة موعد جديد\n\nاختر القسم:</blockquote>",
            reply_markup=InlineKeyboardMarkup(buttons)
        )

    elif data.startswith("ch_sched_add_cat|"):
        # format: ch_sched_add_cat|ch_id|cat_index
        parts = data.split("|", 2)
        ch_id = parts[1] if len(parts) > 1 else ""
        cat_raw = parts[2] if len(parts) > 2 else ""
        cat = CAT_BY_INDEX.get(cat_raw, cat_raw)
        ...
        for i, sid in enumerate(station_ids, 1):
            name = ST_TIMO[sid]["name"].replace("إذاعة ", "").replace("اذاعة ", "").strip()
            row.append(InlineKeyboardButton(f"{name}", callback_data=f"ch_sched_add_pick|{ch_id}|{sid}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
    ...
    elif data.startswith("ch_sched_add_pick|"):
        # format: ch_sched_add_pick|ch_id|sid
        parts = data.split("|", 2)
        ch_id = parts[1] if len(parts) > 1 else ""
        sid = parts[2] if len(parts) > 2 else ""
        await query.answer()
        ...
        await query.edit_message_text(
            ...
            reply_markup=build_time_picker_kb(h, m, "ch_sched_add_time", f"{user_id}|{ch_id}|{sid}")
        )

    elif data.startswith("ch_sched_time_toggle|"):
        await query.answer()
        # format: ch_sched_time_toggle|ch_id|HH:MM
        parts = data.split("|", 2)
        ch_id = parts[1] if len(parts) > 1 else ""
        t = parts[2] if len(parts) > 2 else ""
        # ابحث عن القناة في بيانات صاحبها الأصلي
        owner_id = user_id
        for uid_key, uinfo in user_data.items():
            if ch_id in uinfo.get("channels", {}):
                owner_id = uid_key
    ...
    elif data.startswith("confirm_auto|"):
        # تأكيد إيقاف البث اليدوي وتفعيل الجدول التلقائي
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        ch_id = data.split("|", 1)[1]
        owner_id = user_id
        for uid_key, uinfo in user_data.items():
            if ch_id in uinfo.get("channels", {}):
                owner_id = uid_key
                break
        ch_info_now = user_data.get(owner_id, {}).get("channels", {}).get(ch_id, {})
        if not ch_info_now:
            await query.answer("❌ القناة غير موجودة", show_alert=True)
            return
        ch_title = ch_info_now.get("title", ch_id)
        try:
            pid = ch_info_now.get("process")
            if pid and is_ffmpeg_running(pid):
                subprocess.run(["kill", "-9", str(pid)], timeout=5, check=True)
            user_data[owner_id]["channels"][ch_id].pop("process", None)
        except Exception as e:
            logger.error(f"confirm_auto kill error: {e}")
        user_data[owner_id]["channels"][ch_id]["schedule_enabled"] = True
        user_data[owner_id]["channels"][ch_id].pop("externally_stopped", None)
        current_time = time.strftime("%H:%M", time.gmtime(time.time() + 7200))
        current_station = None
        ch_disabled_set = set(ch_info_now.get("schedule_disabled", []))
        ch_schedule = ch_info_now.get("ch_schedule", AUTO_SCHEDULE)
        for t in reversed(sorted(ch_schedule.keys())):
        ...
        if current_station:
            try:
                ...
                process = subprocess.Popen(ffmpeg_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                user_data[owner_id]["channels"][ch_id]["process"] = process.pid
                user_data[owner_id]["channels"][ch_id]["station"] = current_station["url"]
                user_data[owner_id]["channels"][ch_id]["broadcast_type"] = "auto"
                user_data[owner_id]["channels"][ch_id]["should_broadcast"] = True
                broadcast_fail_count[ch_id] = 0
                reset_broadcast_stats(ch_id, current_station["name"], owner_id, "auto")
            except Exception as e:
                logger.error(f"confirm_auto start error: {e}")
        save_data()
        await query.edit_message_text(
            f"<blockquote>✅ تم تفعيل الجدول التلقائي\n\n"
            f"📢 {ch_title}\n"
            f"🎙 المحطة الحالية: {current_station['name'] if current_station else 'لا توجد محطة'}\n"
            f"📅 {egypt_strftime()}</blockquote>"
        )

    elif data.startswith("confirm_manual|"):
        # تأكيد إيقاف الجدول التلقائي وبدء البث اليدوي
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        parts = data.split("|", 1)
        channel_id = parts[1] if len(parts) > 1 else ""
        # الـ station محفوظ في user_state أو user_data عشان callback_data ميتعداش 64 بايت
        selected_station = user_state.get(user_id, {}).get("pending_manual_station", "")
        # لو مش موجود في user_state (بعد restart) اقرأ من user_data
        if not selected_station:
            selected_station = user_data.get(user_id, {}).get("pending_manual_station", "")
        # ملاحظة 2: تحقق إن البيانات موجودة قبل التشغيل
        if "pending_manual_station" not in user_state.get(user_id, {}) and            "pending_manual_station" not in user_data.get(user_id, {}):
            await query.answer("❌ انتهت صلاحية الطلب، اختر الإذاعة مجدداً", show_alert=True)
            return
        if not selected_station or channel_id not in user_data.get(user_id, {}).get("channels", {}):
            await query.answer("❌ انتهت صلاحية الطلب، اختر الإذاعة مجدداً", show_alert=True)
            return
        ...
        try:
            ...
            logger.error(f"confirm_manual kill error: {e}")
        user_data[user_id]["channels"][channel_id]["schedule_enabled"] = False
        try:
            ffmpeg_cmd = build_ffmpeg_cmd(selected_station, channel_info["rtmps_url"])
            process = subprocess.Popen(ffmpeg_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            user_data[user_id]["channels"][channel_id]["process"] = process.pid
            ...
            # تصفير وقت البدء ومدة البث عند بدء بث جديد
            station_name_r = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), selected_station)
            reset_broadcast_stats(channel_id, station_name_r, user_id, "manual")
            # ملاحظة 1: امسح البيانات بعد الاستخدام مباشرة (del بدل pop)
            user_state.get(user_id, {}).pop("pending_manual_station", None)
            user_state.get(user_id, {}).pop("pending_manual_channel", None)
            if user_id in user_data:
                user_data[user_id].pop("pending_manual_station", None)
                user_data[user_id].pop("pending_manual_channel", None)
            save_data()
            station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), "الإذاعة المختارة")
            await query.edit_message_text(
                f"<blockquote>✅ تم إيقاف الجدول التلقائي وبدء البث اليدوي\n\n"
                f"📢 {ch_title}\n"
                f"🎙 الإذاعة: {station_name}\n"
                f"📅 {egypt_strftime()}</blockquote>"
            )
        except Exception as e:
            logger.error(f"confirm_manual start error: {e}")
            await query.edit_message_text(f"<blockquote>❌ فشل تشغيل البث: {e}</blockquote>")

    elif data.startswith("ch_sched_toggle|"):
        await query.answer()
        # format: ch_sched_toggle|ch_id
        parts = data.split("|", 1)
        ch_id = parts[1] if len(parts) > 1 else ""
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        # ابحث عن صاحب القناة
        ...
        for uid_key, uinfo in user_data.items():
            if ch_id in uinfo.get("channels", {}):
                owner_id = uid_key
                break
        if owner_id not in user_data or ch_id not in user_data[owner_id].get("channels", {}):
            await query.answer("❌ القناة غير موجودة", show_alert=True)
            return
        ch_info_now = user_data[owner_id]["channels"][ch_id]
        current = ch_info_now.get("schedule_enabled", False)
        ch_title = ch_info_now.get("title", ch_id)

        # ━━━ تفعيل الجدول ━━━
        if not current:
            pid = ch_info_now.get("process")
            has_manual = pid and is_ffmpeg_running(pid) and ch_info_now.get("broadcast_type", "manual") != "auto"
            if has_manual:
                # في بث يدوي — اسأل المستخدم يأكد التبديل
                current_station_url = ch_info_now.get("station", "")
                station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == current_station_url), "البث الحالي")
                await query.answer("⚠️ في بث يدوي شغال حالياً", show_alert=False)
                await app.send_message(
                    int(owner_id),
                    f"<blockquote>⚠️ البث اليدوي شغّال على قناة\n📢 {ch_title}\n"
                    f"🎙 الإذاعة الحالية: {station_name}\n\n"
                    f"هل تريد إيقافه وتفعيل الجدول التلقائي؟</blockquote>",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ نعم، فعّل الجدول التلقائي", callback_data=f"confirm_auto|{ch_id}")
                    ]])
                )
                return
            # لا يوجد بث يدوي — شغّل الجدول مباشرة
            user_data[owner_id]["channels"][ch_id]["schedule_enabled"] = True
            # امسح externally_stopped عشان يسمح بالتشغيل حتى لو القناة اتوقفت قبل كده
            user_data[owner_id]["channels"][ch_id].pop("externally_stopped", None)
            current_time = time.strftime("%H:%M", time.gmtime(time.time() + 7200))
            current_station = None
            ch_disabled_set = set(user_data[owner_id]["channels"][ch_id].get("schedule_disabled", []))
            ch_schedule = user_data[owner_id]["channels"][ch_id].get("ch_schedule", AUTO_SCHEDULE)
            ...
            ch_info = user_data[owner_id]["channels"][ch_id]
            txt, kb = _draw_ch_sched(ch_id, ch_info)
            await query.edit_message_text(txt, reply_markup=kb)

        # ━━━ إيقاف الجدول ━━━
        else:
            user_data[owner_id]["channels"][ch_id]["schedule_enabled"] = False
            try:
                pid = ch_info_now.get("process")
                if pid and is_ffmpeg_running(pid):
                    subprocess.run(["kill", "-9", str(pid)], timeout=5, check=True)
                user_data[owner_id]["channels"][ch_id].pop("process", None)
                user_data[owner_id]["channels"][ch_id].pop("broadcast_type", None)
                # المشكلة 6: ضع علامة externally_stopped عشان restore ميعيدش تشغيله
                user_data[owner_id]["channels"][ch_id]["externally_stopped"] = True
                user_data[owner_id]["channels"][ch_id]["should_broadcast"] = False
            except Exception as e:
                logger.error(f"ch_sched_toggle stop error: {e}")
            save_data()
            _owner_int = int(owner_id)
            _main_kb_sched = admin_keyboard(owner_id) if is_admin(_owner_int) else user_keyboard(owner_id)
            try:
                await query.message.delete()
            except Exception:
                pass
        ...
        if page >= total_pages or page < 0:
            ...
            return
        entries = list(broadcast_log)  # من الأقدم للأجدد
        ...
        for i, e in enumerate(chunk, start=1):
            ...
            btype_label = "🤖 تلقائي" if btype == "auto" else "🌙 ليلي" if btype == "night" else "👤 يدوي"
            station = e.get("station", "غير معروف").replace("إذاعة ", "").strip()
            num = i + page * per_page
            # جلب الاسم من user_data لو كان غير معروف أو فاضي
            display_name = e.get("name", "")
            if not display_name or display_name == "غير معروف":
                _euid = str(e.get("user_id", ""))
                display_name = user_data.get(_euid, {}).get("name") or _euid or "غير معروف"
            lines.append(
                f"{num}. ❪ {_fmt_time(e['time'])} ❫\n"
                f"👤 {display_name}  |  📢 {e['channel']}\n"
        ...
        if "process" in channel_info and is_ffmpeg_running(channel_info["process"]):
            from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
            btype_now = channel_info.get("broadcast_type", "manual")
            if btype_now in ("auto", "night"):
                ch_title_c = channel_info.get("title", channel_id)
                station_name_c = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), selected_station)
                btype_label = "الجدول التلقائي" if btype_now == "auto" else "الوضع الليلي"
                user_state[user_id] = user_state.get(user_id, {})
                user_state[user_id]["pending_manual_station"] = selected_station
                user_state[user_id]["pending_manual_channel"] = channel_id
                if user_id in user_data:
                    user_data[user_id]["pending_manual_station"] = selected_station
                    user_data[user_id]["pending_manual_channel"] = channel_id
                    save_data()
                await query.edit_message_text(
                    f"<blockquote>⚠️ {btype_label} شغّال على قناة\n"
                    f"📢 {ch_title_c}\n\n"
                    f"هل تريد إيقافه وبدء البث اليدوي؟\n"
                    f"🎙 الإذاعة المختارة: {station_name_c}</blockquote>",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ نعم، ابدأ البث اليدوي", callback_data=f"confirm_manual|{channel_id}")
                    ]])
                )
            else:
                await query.answer(
        ...
        try:
            ...
            user_data[user_id]["channels"][channel_id]["station"] = selected_station
            user_data[user_id]["channels"][channel_id].pop("externally_stopped", None)
            user_data[user_id]["channels"][channel_id]["should_broadcast"] = True
            broadcast_fail_count[channel_id] = 0
        ...
        reset_broadcast_stats(channel_id, station_name_notify, user_id, "manual")
        save_broadcast_state()
...
async def restore_command(client, message):
    ...
    if not message.reply_to_message or not message.reply_to_message.document:
        await message.reply_text(
            ...
            "• required_channels_backup.json\n"
            "• broadcast_log.json\n"
            "• broadcast_stats.json\n"
            "• broadcast_start_times.json</blockquote>"
        )
        return
    try:
        doc = message.reply_to_message.document
        file_name = doc.file_name
        allowed = ["user_data.json", "settings.json", "required_channels_backup.json", "admins_backup.json", "broadcast_log.json", "broadcast_stats.json", "broadcast_start_times.json"]
        ...
        if file_name == "user_data.json":
            ...
            # احفظ REQUIRED_CHANNELS قبل load_settings عشان متتمسحش
            _saved_rc = list(REQUIRED_CHANNELS)
            load_settings()
            if not REQUIRED_CHANNELS and _saved_rc:
                REQUIRED_CHANNELS[:] = _saved_rc
        ...
        elif file_name == "broadcast_log.json":
            load_broadcast_log_file()
            await message.reply_text(f"<blockquote>✅ تم استعادة سجل البثوث بنجاح 🎉\n📋 {len(broadcast_log)} سجل</blockquote>")
        elif file_name == "broadcast_stats.json":
            load_broadcast_stats_file()
            await message.reply_text(f"<blockquote>✅ تم استعادة إحصائيات البث بنجاح 🎉\n📊 {len(broadcast_stats)} قناة</blockquote>")
        elif file_name == "broadcast_start_times.json":
            load_broadcast_stats_file()
            await message.reply_text("<blockquote>✅ تم استعادة أوقات بدء البث بنجاح 🎉</blockquote>")
    except Exception as e:
...
async def start_command(client, message):
    ...
    try:
        ...
        if is_new_user:
            ...
            # تحديث الاسم دايماً عند كل start للمستخدمين الموجودين
...
async def handle_text(client, message):
    ...
    # تحديث اسم المستخدم في user_data دايماً عند كل تفاعل
    if user_id in user_data and message.from_user.first_name:
        user_data[user_id]["name"] = message.from_user.first_name
        if message.from_user.username:
            user_data[user_id]["username"] = message.from_user.username
    try:
        ...
        if text == "إضافة قناة":
        ...
        elif text == "🔙 رجوع" and user_id in sched_state:  # رجوع من لوحة التحكم
        ...
        elif text in ["✏️ تعديل الوقت", "🎙 تغيير المحطة", "⏹ إيقاف محطة", "🗑 حذف محطة", "➕ إضافة محطة"]:
            # لو sched_state مش موجود (بعد ريستارت) — نطلب من المستخدم يختار القناة
            if user_id not in sched_state:
                from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
                channels = user_data.get(user_id, {}).get("channels", {})
                if not channels:
                    await message.reply_text("<blockquote>❌ لا توجد قنوات مضافة</blockquote>")
                ...
                for cid, cinfo in channels.items():
                    buttons.append([InlineKeyboardButton(
                        f"📢 {cinfo.get('title', cid)}",
                        callback_data=f"ch_sched_open_{cid}"
                    )])
                await message.reply_text(
                    "<blockquote>اختر القناة أولاً:</blockquote>",
                    reply_markup=InlineKeyboardMarkup(buttons)
                )
                return
            ...
            if text == "➕ إضافة محطة":
                ...
                for cat_name in ST_CATEGORIES.keys():
                    if mode == "ch_sched":
                        ...
                        buttons.append([InlineKeyboardButton(cat_name, callback_data=f"unight_add_cat|{uid}|{ch_id}|{CAT_INDEX.get(cat_name, cat_name)}")])
                await message.reply_text(
                ...
                for t, s in sorted(ch_schedule.items()):
                    ...
                    if mode == "ch_sched":
                        is_on = t not in ch_disabled
                        icon = "✅" if is_on else "❌"
                        cb = f"ch_sched_action|{action}|{ch_id}|{t}"
                    else:
                        icon = "🌙"
                        cb = f"unight_action|{action}|{uid}|{ch_id}|{t}"
            ...
            try:
                ...
                if 0 <= choice < len(channels):
                    channel_id, info = channels[choice]
                    ch_title = info.get("title", channel_id)
                    # عرض تأكيد الحذف بدل الحذف المباشر
                    from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
                    await message.reply_text(
                        f"<blockquote>⚠️ تأكيد حذف القناة\n\n📢 {ch_title}\n\nهل أنت متأكد أنك تريد حذف القناة؟</blockquote>",
                        reply_markup=InlineKeyboardMarkup([
            ...
            channels = user_data[user_id]["channels"]
            buttons = [
                [InlineKeyboardButton(f"🗑 {info['title']}", callback_data=f"del_confirm_{ch_id}")]
            ...
            if channels:
                ...
                for ch_id, ch_info in list(channels.items()):
                    ...
                    ch_buttons.append([InlineKeyboardButton(
                        f"{status_icon} {ch_info.get('title', ch_id)}",
                        callback_data=f"ch_sched_toggle|{ch_id}"
                    )])
                await message.reply_text(
            ...
            def _fmt_time2(t):
                try:
                    ...
                    date_part = parts2[0]
                    time_part = parts2[1]
                    period = parts2[2] if len(parts2) > 2 else ""
                    y, mo, d = date_part.split("-")
                    h2, mi2, *_ = time_part.split(":")
                    return f"{int(d)} {_MONTHS2.get(mo, mo)} {y} — {int(h2)}:{mi2} {period}"
                except:
                    return t
            lines = []
            for i, e in enumerate(chunk, start=1):
            ...
            ch_id_ref = user_state.get(user_id, {}).get("refresh_ch_id", "")
            if ch_id_ref:
                # per-channel
                key_ref = f"{user_id}_{ch_id_ref}"
                auto_refresh_enabled[key_ref] = True
                auto_refresh_interval[key_ref] = mins
            else:
                # fallback لو مفيش ch_id
            ...
            try:
                ...
                reset_broadcast_stats(channel_id, station_name_stat, user_id, "manual")
                save_broadcast_state()
                user_state.pop(user_id, None)
                await message.reply_text(
    ...
    if step == "awaiting_report":
        ...
        for uid, uinfo in user_data.items():
            ...
            for ch_id, ch_info in uinfo.get("channels", {}).items():
                ...
                if (uid_match or name_match or ch_match) and "process" in ch_info:
...
def auto_refresh_thread():
    ...
    while True:
        try:
            for key, enabled in list(auto_refresh_enabled.items()):
                if not enabled:
                    continue
                # key ممكن يكون uid أو uid_ch_id
                if "_" in str(key) and not str(key).lstrip("-").isdigit():
                    # per-channel: key = uid_ch_id
                    parts_key = str(key).rsplit("_", 1)
                    uid = parts_key[0]
                    ch_id = parts_key[1]
                    ch_info = user_data.get(uid, {}).get("channels", {}).get(ch_id, {})
                    if "process" not in ch_info or not is_ffmpeg_running(ch_info["process"]):
                        continue
                    interval_mins = auto_refresh_interval.get(key, 30)
                    last = last_refresh.get(key, 0)
                    if time.time() - last >= interval_mins * 60:
                        # أعد تشغيل هذه القناة فقط
                        station = ch_info.get("station")
                        if station:
                            try:
                                if is_ffmpeg_running(ch_info["process"]):
                                    subprocess.run(["kill", "-9", str(ch_info["process"])], timeout=5)
                                ffmpeg_cmd = build_ffmpeg_cmd(station, ch_info["rtmps_url"])
                                process = subprocess.Popen(ffmpeg_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                                user_data[uid]["channels"][ch_id]["process"] = process.pid
                                save_data()
                                last_refresh[key] = time.time()
                                logger.info(f"Auto refresh for channel {ch_id} (user {uid})")
                            except Exception as e:
                                logger.error(f"auto_refresh_thread channel error: {e}")
                else:
                    # per-user (legacy)
                    uid = str(key)
                    user_info = user_data.get(uid, {})
...
async def _auto_restore_user_data():
    """استعادة user_data تلقائياً من آخر رسالة في قناة البيانات عند الـ start"""
    global user_data
    try:
        if os.path.exists(DATA_FILE) and os.path.getsize(DATA_FILE) > 10:
            logger.info("user_data.json موجود — مش محتاج restore تلقائي")
            return False
        logger.info("user_data.json مش موجود — جاري البحث في قناة البيانات...")
        # ابحث في آخر 50 رسالة في قناة البيانات (تلقائية أو يدوية)
        found_msg = None
        async for message in app.get_chat_history(BACKUP_CHANNEL_ID, limit=50):
            if (message.document and 
                message.document.file_name == "user_data.json"):
                found_msg = message
                break
        if not found_msg:
            logger.warning("مش لقيت user_data في قناة البيانات")
            return False
        # حمّل الملف
        await found_msg.download(file_name=DATA_FILE)
        # حمّل في الذاكرة
        with open(DATA_FILE, "r") as f:
            user_data = json.load(f)
        logger.info(f"✅ تم استعادة user_data تلقائياً — {len(user_data)} مستخدم")
        # استعادة باقي الملفات من قناة البيانات
        restore_files = {
            "required_channels_backup.json": "/data/required_channels_backup.json",
            "admins_backup.json":            "/data/admins_backup.json",
            "settings.json":                 "/data/settings.json",
            "broadcast_state.json":          "/data/broadcast_state.json",
            "broadcast_stats.json":          "/data/broadcast_stats.json",
        }
        found_files = {}
        try:
            async for message in app.get_chat_history(BACKUP_CHANNEL_ID, limit=100):
                if message.document and message.document.file_name in restore_files:
                    fname = message.document.file_name
                    if fname not in found_files:
                        found_files[fname] = message
                if len(found_files) == len(restore_files):
                    break
        except Exception as e:
            logger.error(f"auto_restore scan error: {e}")
        for fname, msg in found_files.items():
            try:
                dest = restore_files[fname]
                await msg.download(file_name=dest)
                logger.info(f"✅ تم استعادة {fname}")
            except Exception as e:
                logger.error(f"auto_restore {fname} error: {e}")
        # تحميل في الذاكرة
        load_required_channels()
        load_admins()
        load_settings()
        ...
        logger.error(f"auto_restore_user_data error: {e}")
        return False
...
async def main():
    ...
    # استعادة تلقائية لو user_data مش موجود
    restored = await _auto_restore_user_data()
    if restored:
        logger.info("✅ تم استعادة البيانات تلقائياً من قناة البيانات")
    # أوامر للجميع
