...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        await query.answer()
        active = []
        # لو أدمن يعرض كل القنوات، لو مستخدم عادي يعرض قنواته بس
        if is_admin(int(user_id)) and has_perm(int(user_id), "stats"):
            users_to_check = user_data.items()
        else:
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🧹 التنظيف التلقائي: " + ("إيقاف ✅" if auto_cleanup_enabled else "تفعيل ❌"), callback_data="toggle_auto_cleanup")],
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("📅 تعديل موعد التقرير الأسبوعي", callback_data="set_weekly_report_sched")],
