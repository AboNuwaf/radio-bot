...
async def send_subscription_message(message, channel):
    ...
    await message.reply_text(
        text,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 " + channel_name + " | " + channel_url, url=channel_url)]
        ])
    )
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        old_time = data[10:]
        user_state[user_id] = {"step": "awaiting_new_time", "old_time": old_time, "timepick_type": "sched"}
        await query.answer()
        station_name = AUTO_SCHEDULE.get(old_time, {}).get("name", "")
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        labels_am = ["١٢ص", "١ص", "٢ص", "٣ص", "٤ص", "٥ص", "٦ص",
                     "٧ص", "٨ص", "٩ص", "١٠ص", "١١ص"]
        labels_pm = ["١٢م", "١م", "٢م", "٣م", "٤م", "٥م", "٦م",
                     "٧م", "٨م", "٩م", "١٠م", "١١م"]
        am_btns = [InlineKeyboardButton(labels_am[i], callback_data=f"timepick_h_{i}_sched") for i in range(12)]
        pm_btns = [InlineKeyboardButton(labels_pm[i], callback_data=f"timepick_h_{i+12}_sched") for i in range(12)]
        hour_rows = []
        for i in range(0, 12, 4):
            hour_rows.append(am_btns[i:i+4])
        for i in range(0, 12, 4):
            hour_rows.append(pm_btns[i:i+4])
        await query.edit_message_text(
            f"<blockquote>✏️ تعديل موعد\n\nالوقت الحالي: {old_time}\nالمحطة: {station_name}\n\n🕐 اختار الساعة الجديدة</blockquote>",
        ...
        await query.answer()
        user_state[str(query.from_user.id)] = {"step": "awaiting_backup_time", "timepick_type": "bsched"}
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        hour_rows = []
        labels_am = ["١٢ص", "١ص", "٢ص", "٣ص", "٤ص", "٥ص", "٦ص",
                     "٧ص", "٨ص", "٩ص", "١٠ص", "١١ص"]
        labels_pm = ["١٢م", "١م", "٢م", "٣م", "٤م", "٥م", "٦م",
                     "٧م", "٨م", "٩م", "١٠م", "١١م"]
        am_btns = [InlineKeyboardButton(labels_am[i], callback_data=f"timepick_h_{i}_bsched") for i in range(12)]
        pm_btns = [InlineKeyboardButton(labels_pm[i], callback_data=f"timepick_h_{i+12}_bsched") for i in range(12)]
        for i in range(0, 12, 4):
            hour_rows.append(am_btns[i:i+4])
        for i in range(0, 12, 4):
            hour_rows.append(pm_btns[i:i+4])
        await query.message.reply_text(
            "<blockquote>🕐 اختار الساعة</blockquote>",
            reply_markup=InlineKeyboardMarkup(hour_rows)
        )

    elif data.startswith("timepick_h_"):
        # e.g. timepick_h_8_bsched  or  timepick_h_20_sched
        await query.answer()
        parts = data.split("_")
        h = int(parts[2])
        ttype = parts[3]
        user_state[str(query.from_user.id)]["timepick_hour"] = h
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        # عرض أزرار الدقائق
        mins = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]
        min_btns = [InlineKeyboardButton(f":{str(m).zfill(2)}", callback_data=f"timepick_m_{m}_{ttype}") for m in mins]
        min_rows = [min_btns[i:i+4] for i in range(0, len(min_btns), 4)]
        # حساب التسمية للعرض
        h12 = h % 12 or 12
        period = "ص" if h < 12 else "م"
        await query.edit_message_text(
            f"<blockquote>🕐 الساعة: {h12} {period}\n\nاختار الدقائق</blockquote>",
            reply_markup=InlineKeyboardMarkup(min_rows)
        )

    elif data.startswith("timepick_m_"):
        await query.answer()
        parts = data.split("_")
        m = int(parts[2])
        ttype = parts[3]
        uid = str(query.from_user.id)
        h = user_state.get(uid, {}).get("timepick_hour", 0)
        h12 = h % 12 or 12
        period = "صباحاً" if h < 12 else "مساءً"
        time_24 = f"{h:02d}:{m:02d}"
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        await query.edit_message_text(
            f"<blockquote>🕐 الوقت المختار: {h12}:{m:02d} {period}\n\nتأكيد؟</blockquote>",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"✅ تأكيد — {h12}:{m:02d} {period}", callback_data=f"timepick_confirm_{h}_{m}_{ttype}")],
                [InlineKeyboardButton("🔄 تغيير", callback_data=f"bsched_time" if ttype == "bsched" else f"edit_time_{user_state.get(uid, {}).get('old_time', '')}")]
            ])
        )

    elif data.startswith("timepick_confirm_"):
        await query.answer()
        globals()["backup_schedule_time"] = globals().get("backup_schedule_time", "08:00")
        parts = data.split("_")
        h = int(parts[2])
        m = int(parts[3])
        ttype = parts[4]
        ...
        if ttype == "bsched":
            backup_schedule_time = time_24
            save_settings()
            user_state.pop(uid, None)
            await query.edit_message_text(
                f"<blockquote>✅ تم تغيير وقت النسخ التلقائي إلى {h12}:{m:02d} {period}</blockquote>"
            )
        elif ttype == "sched":
            old_time = user_state.get(uid, {}).get("old_time")
            if old_time and old_time in AUTO_SCHEDULE:
                station_data = AUTO_SCHEDULE.pop(old_time)
                AUTO_SCHEDULE[time_24] = station_data
                AUTO_SCHEDULE.update(dict(sorted(AUTO_SCHEDULE.items())))
                save_settings()
            user_state.pop(uid, None)
            await query.edit_message_text(
                f"<blockquote>✅ تم تغيير الموعد\nمن: {old_time}\nإلى: {h12}:{m:02d} {period}</blockquote>"
            )
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            await message.reply_text(
                ...
                "أرسل معرف قناتك بالصيغ التالية:\n\n"
                "• @اسم_القناة\n"
                "• https://t.me/اسم_القناة\n\n"
