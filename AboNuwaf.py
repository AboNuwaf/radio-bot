...
async def send_broadcast_notification(client, chat_id, station_url, user_id=None):
    # تحقق من إعداد المستخدم الشخصي فقط
    ...
    if data == "clear_reviews":
        ...
        for path, label in files:
            if os.path.exists(path):
                ...
                sent_count += 1
        await query.message.reply_text(
            f"<blockquote>✅ تم إرسال {sent_count} ملف\n\n"
            "💡 احفظهم في مكان آمن</blockquote>"
        )
        ...
        global backup_schedule_day
        await query.answer()
        backup_schedule_day = int(data.split("_")[-1])
        save_settings()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        sched_status = f"✅ {DAYS_AR.get(backup_schedule_day, '')} الساعة {backup_schedule_time}" if backup_schedule_enabled else "❌ موقوف"
        days_buttons = [
        ...
        global backup_schedule_enabled
        await query.answer()
        backup_schedule_enabled = not backup_schedule_enabled
        save_settings()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        sched_status = f"✅ {DAYS_AR.get(backup_schedule_day, '')} الساعة {backup_schedule_time}" if backup_schedule_enabled else "❌ موقوف"
        days_buttons = [
        ...
        await query.answer()
        user_state[str(query.from_user.id)] = {"step": "awaiting_backup_time"}
        await query.message.reply_text(
            "<blockquote>🕐 أرسل الوقت بالصيغة التالية:\n\nمثال: 08:00 أو 20:30</blockquote>",
            reply_markup=ReplyKeyboardMarkup([["إلغاء"]], resize_keyboard=True)
        )
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
        ...
        elif user_state.get(user_id, {}).get("step") == "awaiting_backup_time":
            if text == "إلغاء":
                user_state.pop(user_id, None)
            ...
            if not (0 <= h <= 23 and 0 <= m <= 59):
                await message.reply_text("<blockquote>❌ وقت غير صحيح</blockquote>")
                return
            global backup_schedule_time
            backup_schedule_time = f"{h:02d}:{m:02d}"
            save_settings()
            user_state.pop(user_id, None)
            await message.reply_text(
                f"<blockquote>✅ تم تغيير وقت النسخ التلقائي إلى {backup_schedule_time}</blockquote>",
                reply_markup=admin_keyboard(user_id)
            )
            if text == "إلغاء":
                user_state.pop(user_id, None)
