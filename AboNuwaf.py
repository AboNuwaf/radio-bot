        ...
        try:
            for user_id, user_info in list(user_data.items()):
                ...
                for channel_id, channel_info in list(channels.items()):
                    ...
                    pid = channel_info["process"]
                    if is_ffmpeg_running(pid):
                        # ✅ البث شغال — صفّر الـ fail_count عشان ميتراكمش
                        broadcast_fail_count.pop(channel_id, None)
                        continue
                    # ❌ ffmpeg وقف فعلاً
    ...
    try:
        ...
        for uid, user_info in sorted_users:
            ...
            for channel_id, channel_info in list(user_info["channels"].items()):
                ...
                # اسم المحطة من broadcast_stats لو موجود — أحدث وأدق
                stat_station = broadcast_stats.get(channel_id, {}).get("station", "")
                if is_active and stat_station:
                    station_name = stat_station
    ...
    if data == "clear_reviews":
        ...
        if ttype == "bsched":
            ...
            if _uid in user_data and _ch_id in user_data[_uid].get("channels", {}):
                ...
                save_data()
            await query.edit_message_text(
                f"<blockquote>✅ تم تغيير الوقت من {_old_t} إلى {h12}:{m:02d} {period}</blockquote>"
            )
            ...
            await query.edit_message_text(
                f"<blockquote>✅ تم إضافة الموعد\n\n🎙 {station_name}\n⏰ الساعة {h12}:{m:02d} {period}</blockquote>"
            )
        ...
        for t, entry in sorted(night_schedule.items()):
            ...
            sched_buttons.append([
                InlineKeyboardButton("✏️ الوقت", callback_data=f"unight_edit_time_{uid}_{ch_id}_{t}"),
                InlineKeyboardButton("🎙 المحطة", callback_data=f"unight_edit_station_{uid}_{ch_id}_{t}"),
                InlineKeyboardButton("🗑 حذف", callback_data=f"unight_del_entry_{uid}_{ch_id}_{t}"),
            ])
        await query.edit_message_text(
            f"<blockquote>🌙 الوضع الليلي\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                sched_buttons + [
                    [InlineKeyboardButton("❌ إيقاف الوضع الليلي" if ch_night else "✅ تفعيل الوضع الليلي",
                                         callback_data=f"unight_toggle_{uid}_{ch_id}")],
                    [InlineKeyboardButton("➕ إضافة موعد", callback_data=f"unight_add_sched_{uid}_{ch_id}")],
                    [InlineKeyboardButton("🔙 رجوع", callback_data=f"open_night_mode_{uid}")],
                ]
            )
        )
        ...
        for tt, entry in sorted(remaining.items()):
            if isinstance(entry, dict):
                ...
                sname = ST_TIMO.get(entry, {}).get("name", entry).replace("إذاعة ", "").strip()
            sched_buttons.append([InlineKeyboardButton(f"🌙 {to_ar_time(tt)} — {sname}", callback_data="noop")])
            sched_buttons.append([
                InlineKeyboardButton("✏️ الوقت", callback_data=f"unight_edit_time_{uid}_{ch_id}_{tt}"),
                InlineKeyboardButton("🎙 المحطة", callback_data=f"unight_edit_station_{uid}_{ch_id}_{tt}"),
                InlineKeyboardButton("🗑 حذف", callback_data=f"unight_del_entry_{uid}_{ch_id}_{tt}"),
            ])
        await query.edit_message_text(
            f"<blockquote>🌙 الوضع الليلي\n📢 {ch_title}\n\nالحالة: {status}\n\n✅ تم حذف الموعد</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                sched_buttons + [
                    [InlineKeyboardButton("❌ إيقاف الوضع الليلي" if ch_night else "✅ تفعيل الوضع الليلي",
                                         callback_data=f"unight_toggle_{uid}_{ch_id}")],
        ...
        # اسم المحطة: من broadcast_stats لو شغال (أحدث)، غير كده من station المحفوظة
        if is_ch_active:
            station_name = broadcast_stats.get(channel_id, {}).get("station", "")
            if not station_name:
                station_url = info.get("station", user_data[uid].get("temp_station", ""))
                station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == station_url), "لا يوجد")
        else:
            station_url = info.get("station", "")
            station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == station_url), "لا يوجد") if station_url else "—"
        ...
        broadcast_count = sum(1 for ch in user_info.get("channels", {}).values() if "process" in ch)
        join_date = user_info.get("join_date", "غير معروف")
        last_seen = user_info.get("last_seen", "غير معروف")
        ...
        for t, s in sorted(ch_schedule.items()):
            ...
            time_buttons.append([
                InlineKeyboardButton("✏️ الوقت", callback_data=f"ch_sched_edit_time_{ch_id}_{t}"),
                InlineKeyboardButton("🎙 المحطة", callback_data=f"ch_sched_edit_station_{ch_id}_{t}"),
                InlineKeyboardButton("🗑 حذف", callback_data=f"ch_sched_del_{ch_id}_{t}"),
                InlineKeyboardButton("⏹" if is_on else "▶️", callback_data=f"ch_sched_time_toggle_{ch_id}_{t}"),
            ])
        await query.edit_message_text(
            f"<blockquote>📅 جدول القناة\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                time_buttons + [
                    [InlineKeyboardButton("➕ إضافة موعد", callback_data=f"ch_sched_add_{ch_id}")],
                    [InlineKeyboardButton(
                        "❌ إيقاف الجدول" if ch_enabled else "✅ تفعيل الجدول",
                        callback_data=f"ch_sched_toggle_{ch_id}"
                    )],
                    [InlineKeyboardButton("🔙 رجوع", callback_data="open_auto_schedule")],
                ]
            )
        )
        ...
        for tt, s in sorted(ch_schedule.items()):
            ...
            time_buttons.append([
                InlineKeyboardButton("✏️ الوقت", callback_data=f"ch_sched_edit_time_{ch_id}_{tt}"),
                InlineKeyboardButton("🎙 المحطة", callback_data=f"ch_sched_edit_station_{ch_id}_{tt}"),
                InlineKeyboardButton("🗑 حذف", callback_data=f"ch_sched_del_{ch_id}_{tt}"),
                InlineKeyboardButton("⏹" if is_on else "▶️", callback_data=f"ch_sched_time_toggle_{ch_id}_{tt}"),
            ])
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        await query.edit_message_text(
            f"<blockquote>📅 جدول القناة\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                time_buttons + [
                    [InlineKeyboardButton("➕ إضافة موعد", callback_data=f"ch_sched_add_{ch_id}")],
                    [InlineKeyboardButton("❌ إيقاف الجدول" if ch_enabled else "✅ تفعيل الجدول",
                                         callback_data=f"ch_sched_toggle_{ch_id}")],
                    [InlineKeyboardButton("🔙 رجوع", callback_data="open_auto_schedule")],
                ]
            )
        ...
        ch_title = ch_info.get("title", ch_id)
        time_buttons = []
        for t2, s in sorted(AUTO_SCHEDULE.items()):
            is_on = t2 not in ch_disabled
            name = s["name"].replace("إذاعة ", "").strip()
            icon = "✅" if is_on else "❌"
            time_buttons.append([
                InlineKeyboardButton(f"{icon} {to_ar_time(t2)} — {name}",
                    callback_data=f"ch_sched_time_toggle_{ch_id}_{t2}")
            ])
        await query.edit_message_reply_markup(InlineKeyboardMarkup(
            time_buttons + [
                [InlineKeyboardButton(
        ...
        ch_enabled = ch_info.get("schedule_enabled", False)
        status = "✅ مفعّل" if ch_enabled else "❌ موقوف"
        ch_schedule = ch_info.get("ch_schedule", AUTO_SCHEDULE)
        ch_disabled = set(ch_info.get("schedule_disabled", []))
        def to_ar_time(t):
            h, m = map(int, t.split(":"))
            period = "صباحاً" if h < 12 else "مساءً"
        ...
        for t, s in sorted(ch_schedule.items()):
            ...
            name = s["name"].replace("إذاعة ", "").strip() if isinstance(s, dict) else t
            icon = "✅" if is_on else "❌"
            time_buttons.append([InlineKeyboardButton(f"{icon} {to_ar_time(t)} — {name}", callback_data="noop")])
            time_buttons.append([
                InlineKeyboardButton("✏️ الوقت", callback_data=f"ch_sched_edit_time_{ch_id}_{t}"),
                InlineKeyboardButton("🎙 المحطة", callback_data=f"ch_sched_edit_station_{ch_id}_{t}"),
                InlineKeyboardButton("🗑 حذف", callback_data=f"ch_sched_del_{ch_id}_{t}"),
        ...
        text_log = (
            ...
            f"الإجمالي: {total} بث | الصفحة {page + 1}/{total_pages}\n"
            f"━━━━━━━━━━━━━━━━\n\n"
            + "\n\n".join(lines)
            + "\n\n━━━━━━━━━━━━━━━━\n"
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            for channel_id, info in list(channels.items()):
                ...
                # اسم المحطة: من broadcast_stats لو شغال، غير كده من station المحفوظة
                if is_ch_active:
                    ch_station_name = broadcast_stats.get(channel_id, {}).get("station", "")
                    if not ch_station_name:
                        ch_station_url = info.get("station", "")
                        ch_station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == ch_station_url), "لا يوجد")
                else:
                    ch_station_name = "متوقف"
                buttons.append([InlineKeyboardButton(
