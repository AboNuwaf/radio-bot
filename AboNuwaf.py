...
def build_time_picker_kb(h, m, ttype="sched", old_time=""):
    ...
    h12 = h % 12 or 12
    period = "🌅 صباحاً" if h < 12 else "🌙 مساءً"
    # نستخدم | كـ separator بين h, m, ttype, old_time عشان ttype وold_time قد يحتووا على _
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        try:
            ...
            # لو كان بث تلقائي — وقف الجدول ولا ترجعله
            if was_auto:
        ...
        if len(active_channels) == 1:
            ...
            was_auto = ch_info.get("broadcast_type") == "auto"
            ch_title = ch_info.get("title", ch_id)
            try:
        ...
        for t, entry in sorted(night_schedule.items()):
            ...
            is_on = t not in set(ch_info.get("night_disabled", []))
            icon = "✅" if is_on else "❌"
            btn_label = "⏹ إيقاف" if is_on else "▶️ تشغيل"
            sched_buttons.append([
                InlineKeyboardButton(f"{icon} {to_ar_time(t)} | {sname}", callback_data="noop")
            ])
            sched_buttons.append([
                InlineKeyboardButton(btn_label, callback_data=f"unight_toggle_entry|{uid}|{ch_id}|{t}"),
            ])
        night_toggle_label = "❌ إيقاف الوضع الليلي" if ch_night else "✅ تفعيل الوضع الليلي"
        await query.edit_message_text(
            f"<blockquote>🌙 الوضع الليلي\n📢 {ch_title}\n\nالحالة: {status}</blockquote>",
        ...
        # format: confirm_manual|channel_id|station_url
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        parts = data.split("|", 2)
        ...
        if not current:
            ...
            if current_station and not ch_info_now.get("externally_stopped", False):
                try:
                    ...
                    logger.error(f"ch_sched_toggle start error: {e}")
        ...
        if "process" in channel_info and is_ffmpeg_running(channel_info["process"]):
            if channel_info.get("broadcast_type") == "auto":
                # الجدول التلقائي شغال — عدّل الرسالة الحالية وأضف زر تأكيد
                ch_title = channel_info.get("title", channel_id)
                station_name = next((v["name"] for v in ST_TIMO.values() if v["url"] == selected_station), selected_station)
                await query.edit_message_text(
                    f"<blockquote>⚠️ الجدول التلقائي شغّال على قناة\n"
                    f"📢 {ch_title}\n\n"
                    f"هل تريد إيقافه وبدء البث اليدوي؟\n"
                    f"🎙 الإذاعة المختارة: {station_name}</blockquote>",
                    reply_markup=InlineKeyboardMarkup([[
