...
def user_keyboard(user_id=None):
    ...
    auto_btn = f"🔄 تلقائي: مفعّل ({interval}د) 🟢" if auto_ref else "🔄 تحديث تلقائي: مقفول 🔴"
    rows = [
        ["إضافة قناة", "قنواتي"],
        ["بدء البث", "إيقاف البث"],
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            await message.reply_text("<blockquote>🔃 تم تحديث بثوثك بنجاح</blockquote>")
            ...
            minutes_map = {"15 دقيقة": 15, "30 دقيقة": 30, "60 دقيقة": 60, "120 دقيقة": 120}
            if text == "إلغاء":
                user_state.pop(user_id, None)
                await message.reply_text(
                    "<blockquote>❌ تم إلغاء العملية</blockquote>",
