...
def restore_broadcasts():
    ...
    for channel_id, info in saved_broadcasts.items():
        try:
            ...
            # المشكلة 1: لو القناة كانت متوقفة يدوياً قبل الـ restart — لا تعيد تشغيلها
            if ch_data.get("externally_stopped", False):
                logger.info(f"Skipping restore for {channel_id} — was manually stopped")
                continue
            # كمان لو مفيش process في user_data — يعني كان متوقف
            if "process" not in ch_data and not info.get("was_running", True):
                logger.info(f"Skipping restore for {channel_id} — no active process")
                continue
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        if len(active_channels) == 1:
            ...
            was_auto_s = ch_info.get("broadcast_type") == "auto"
            was_night_s = ch_info.get("broadcast_type") == "night"
            try:
            ...
            _uid_int2 = int(uid)
            _main_kb2 = admin_keyboard(uid) if is_admin(_uid_int2) else user_keyboard(uid)
            if was_auto_s:
                await query.edit_message_text(
                    f"<blockquote>⏹ تم إيقاف الجدول التلقائي والإذاعة\n\n"
