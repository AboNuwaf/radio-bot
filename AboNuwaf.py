...
broadcast_stats = {}
saved_broadcasts = {}
_last_auto_backup = 0  # وقت آخر auto backup للـ user_data
banned_users = set()
...
def save_broadcast_state():
    ...
    for user_id, user_info in list(user_data.items()):
        channels = user_info.get("channels", {})
        for channel_id, channel_info in list(channels.items()):
            # حفظ القنوات المتوقفة يدوياً عشان نتجنب إعادة تشغيلها بعد restart
            if channel_info.get("externally_stopped", False):
...
def restore_broadcasts():
    ...
    # عشان لو user_data.json اتحفظ وهو شغال، مش نشغل القناة تاني
    # المرجع الأساسي هو broadcast_state.json بس
    ...
    for channel_id, info in saved_broadcasts.items():
        try:
            ...
            # لو القناة كانت متوقفة يدوياً — لا تعيد تشغيلها
            if ch_data.get("externally_stopped", False):
                logger.info(f"Skipping restore for {channel_id} — was manually stopped")
                continue
            # لو broadcast_state صريح إن was_running = False — لا تعيد تشغيلها
            if not info.get("was_running", False):
                logger.info(f"Skipping restore for {channel_id} — was_running is False")
                continue
            # ━━━ الحل الجذري ━━━
            # user_data هو المرجع الأساسي — لو مفيش process فيه يعني القناة كانت موقفة
            # حتى لو broadcast_state قال was_running=True (ممكن يكون قديم من قبل الإيقاف)
            ch_has_schedule = ch_data.get("schedule_enabled", False) or ch_data.get("night_mode_enabled", False)
            if "process" not in ch_data and not ch_has_schedule:
                logger.info(f"Skipping restore for {channel_id} — no active process in user_data (stopped before upload)")
                continue
            # لو في جدول بس مفيش process — شغّل من الجدول بس لو مش موقف يدوياً
            # لو مفيش process ومفيش جدول — وقف خالص
            if "process" not in ch_data and ch_has_schedule:
                # جدول موجود — هيشتغل من run_auto_schedule عادي، مش محتاج restore
                logger.info(f"Skipping restore for {channel_id} — has schedule, will start via scheduler")
                continue
            # تحقق إن الـ process القديم مش شغال فعلاً — لو شغال يعني البوت لسه شغال (مش restart حقيقي)
...
def save_data():
    ...
    # trigger backup لو فات أكتر من 4 ساعات من آخر backup
    global _last_auto_backup
    if _bot_loop and time.time() - _last_auto_backup > 14400:
        try:
            asyncio.run_coroutine_threadsafe(_auto_backup_user_data(), _bot_loop)
        except Exception:
            pass
...
def save_settings():
    ...
    try:
        settings = {
            ...
            "last_auto_backup_time": _last_auto_backup,
        }
...
def load_settings():
    ...
    try:
        ...
        global _last_auto_backup
        _last_auto_backup = s.get("last_auto_backup_time", 0)
...
_auto_backup_running = False  # منع التشغيل المتزامن

async def _auto_backup_user_data():
    """حفظ user_data تلقائياً في قناة البيانات — مرة واحدة كل 4 ساعات"""
    global _last_auto_backup, _auto_backup_running
    if _auto_backup_running:
        logger.info("Auto backup already running — skipped")
        return None
    ...
    try:
        with data_lock:
            with open(DATA_FILE, "w") as f:
                json.dump(user_data, f, ensure_ascii=False, indent=4)
        size_kb = round(os.path.getsize(DATA_FILE) / 1024, 1)
        msg = await app.send_document(
            BACKUP_CHANNEL_ID,
            document=DATA_FILE,
            caption=f"<blockquote>🔄 نسخة تلقائية — user_data\n"
                    f"👥 {len(user_data)} مستخدم\n"
                    f"📁 {size_kb} KB\n"
                    f"📅 {egypt_strftime()}</blockquote>"
        )
        # required_channels
        rc_path = "/data/required_channels_backup.json"
        ...
        if os.path.exists(rc_path):
            await app.send_document(
                BACKUP_CHANNEL_ID,
                document=rc_path,
                caption=f"<blockquote>🔄 نسخة تلقائية — required_channels\n"
                        f"📢 {len(REQUIRED_CHANNELS)} قناة\n"
                        f"📅 {egypt_strftime()}</blockquote>"
            )
        # admins_backup
        admins_path = "/data/admins_backup.json"
        save_admins()
        if os.path.exists(admins_path):
            await app.send_document(
                BACKUP_CHANNEL_ID,
                document=admins_path,
                caption=f"<blockquote>🔄 نسخة تلقائية — admins\n"
                        f"👑 {len(ADMIN_ID) - 1} أدمن\n"
                        f"📅 {egypt_strftime()}</blockquote>"
            )
        # settings
        ...
        if os.path.exists(settings_path):
            await app.send_document(
                ...
                caption=f"<blockquote>🔄 نسخة تلقائية — settings\n"
                        f"📅 {egypt_strftime()}</blockquote>"
            )
        # broadcast_state
        state_path = "/data/broadcast_state.json"
        save_broadcast_state()
        if os.path.exists(state_path):
            await app.send_document(
                BACKUP_CHANNEL_ID,
                document=state_path,
                caption=f"<blockquote>🔄 نسخة تلقائية — broadcast_state\n"
                        f"📅 {egypt_strftime()}</blockquote>"
            )
        # broadcast_stats
        stats_path = "/data/broadcast_stats.json"
        save_broadcast_stats_file()
        if os.path.exists(stats_path):
            await app.send_document(
                BACKUP_CHANNEL_ID,
                document=stats_path,
                caption=f"<blockquote>🔄 نسخة تلقائية — broadcast_stats\n"
                        f"📅 {egypt_strftime()}</blockquote>"
            )
        _last_auto_backup = time.time()
        logger.info(f"✅ Auto backup sent to {BACKUP_CHANNEL_ID}")
        return msg
    except Exception as e:
        logger.error(f"auto_backup_user_data error: {e}")
        return None
    finally:
...
def _auto_backup_thread():
    """thread يبعت backup كل 4 ساعات"""
    while True:
        try:
            time.sleep(60)  # فحص كل دقيقة
            elapsed = time.time() - _last_auto_backup
            if elapsed >= 14400:  # 4 ساعات
                if _bot_loop:
                    asyncio.run_coroutine_threadsafe(_auto_backup_user_data(), _bot_loop)
        except Exception as e:
            logger.error(f"_auto_backup_thread error: {e}")
