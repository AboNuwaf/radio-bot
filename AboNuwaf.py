...
def save_required_channels():
    ...
    try:
        ...
        with open("required_channels_backup.json", "w") as f:
...
def load_required_channels():
    ...
    try:
        ...
        if val and val != "[]":
            ...
            return
        if os.path.exists("required_channels_backup.json"):
            with open("required_channels_backup.json", "r") as f:
...
AUTO_SCHEDULE = {
    ...
    "16:00": {"name": "إذاعة المنشاوي - مرتل", "url": "https://qurango.net/radio/minshawi_murattal"},
...
ST_TIMO = {
    ...
    "2": {"name": "إذاعة القرآن من مكة المكرمة", "url": "https://stream.radiojar.com/0tpy1h0kxtzuv"},
    ...
    "9": {"name": "إذاعة الحرم المكي", "url": "https://qurango.net/radio/makkah_live"},
    ...
    "20": {"name": "إذاعة تلاوة الشيخ حسن صالح", "url": "https://qurango.net/radio/hassan_saleh"},
    ...
    "24": {"name": "إذاعة المنشاوي - مرتل", "url": "https://qurango.net/radio/minshawi_murattal"},
    "25": {"name": "إذاعة خالد الجليل", "url": "https://qurango.net/radio/khalid_aljileel"},
    "26": {"name": "إذاعة محمود الحصري - مرتل", "url": "https://qurango.net/radio/husary_murattal"},
...
def load_settings():
    ...
    global all_reports, all_bot_reviews, subscription_violations
...
def admin_keyboard():
    ...
    return ReplyKeyboardMarkup([
        ...
        [max_btn, report_btn],
        [night_btn],
        [channel_notify_btn, notify_btn],
        ["ℹ️ نبذة عن البوت", about_btn],
        ["🛠 الدعم الفني"],
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        text = (
            ...
            f"التقييم الحالي: {get_rating_stars(current)} ({current}/10)\n"
        ...
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("⭐6", callback_data=f"give_rate_{sid}_6"),
             InlineKeyboardButton("⭐7", callback_data=f"give_rate_{sid}_7"),
             InlineKeyboardButton("⭐8", callback_data=f"give_rate_{sid}_8"),
             InlineKeyboardButton("⭐9", callback_data=f"give_rate_{sid}_9"),
             InlineKeyboardButton("⭐10", callback_data=f"give_rate_{sid}_10")],
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            save_required_channels()
            user_state.pop(user_id, None)
            txt = f"<blockquote>✅ تم إضافة {channel_id_add} بنجاح."
            if custom_text_add:
                preview = custom_text_add[:50] + "..." if len(custom_text_add) > 50 else custom_text_add
                txt += f"\n📝 النص: {preview}"
            txt += "</blockquote>"
            await message.reply_text(txt, reply_markup=admin_keyboard())
            ...
            if found:
                ...
                if new_text:
                    ...
                    await message.reply_text(f"<blockquote>✅ تم تحديث نص {ch_id_edit}\n📝 النص: {preview}</blockquote>", reply_markup=admin_keyboard())
                else:
                    await message.reply_text(f"<blockquote>✅ تم حذف النص المخصص من {ch_id_edit}</blockquote>", reply_markup=admin_keyboard())
            else:
                await message.reply_text("<blockquote>❌ لم يتم العثور على القناة</blockquote>", reply_markup=admin_keyboard())
            ...
            from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
            if int(user_id) == OWNER_ID:
                # المطور يشوف كل البلاغات
                if not all_reports:
                    await message.reply_text("<blockquote>🛠 مركز الدعم الفني\n\n📭 لا توجد بلاغات حتى الآن</blockquote>")
                else:
            ...
            await message.reply_text(
                ...
                f"📻 {station_name}\n"
                f"التقييم الجديد: {get_rating_stars(new_avg)} ({new_avg}/10)\n"
