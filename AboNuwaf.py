...
@web_app.route('/')
def home():
    return "بوت راديو القرآن الكريم شغال!"
...
sys.stdout.flush()
keep_alive()
print(f"✅ خادم الويب شغال على منفذ: {os.environ.get('PORT', 8080)}")
sys.stdout.flush()
...
API_ID = os.environ.get("29667286")  # آمن، القيمة مش موجودة هنا
API_HASH = os.environ.get("2dddc2f98e16161cb50e41971f9591be")  # آمن، القيمة مش موجودة هنا
BOT_TOKEN = os.environ.get("7795185106:AAGMAOVgGchw-YEKWWHR_DEyBINpGMvOGNY")  # آمن، القيمة مش موجودة هنا
OWNER_ID = 6869497898  # آمن، ده مجرد رقم")
...
REQUIRED_CHANNELS = [] # تم تعطيل الاشتراك الإجباري بإفراغ القائمة دون حذف الكود الأصلي
...
MAINTENANCE_IMAGE = "https://ibb.co/nsmph6Hp"
...
async def main():
    ...
    import os
    if os.path.exists('radio_bot.session'):
        os.remove('radio_bot.session')
        logger.info('Session file deleted, starting fresh...')
    await app.start()
...
if __name__ == "__main__":
    ...
    asyncio.run(main())
# ============================================
# 🛠 إضافة ميزة التحكم في قنوات الاشتراك الإجباري
# (هذا الكود تمت إضافته دون تعديل أي سطر أصلي في الملف)
# ============================================
...
async def add_required_channel(client, message):
    ...
    if len(args) < 3:
        await message.reply_text("❌ الاستخدام: `/add_channel @username رابط_القناة`\nمثال: `/add_channel @SEVO_TEAM https://t.me/SEVO_TEAM`")
        return
    
    channel_id = args[1]
    channel_url = args[2]
    
    # التحقق إذا كانت القناة موجودة مسبقاً
    for ch in REQUIRED_CHANNELS:
        if ch["id"] == channel_id:
            await message.reply_text("⚠️ هذه القناة موجودة بالفعل في قائمة الاشتراك الإجباري.")
            return
            
    REQUIRED_CHANNELS.append({"id": channel_id, "url": channel_url})
    await message.reply_text(f"✅ تم إضافة القناة {channel_id} إلى قائمة الاشتراك الإجباري بنجاح.")
...
async def del_required_channel(client, message):
    ...
    if len(REQUIRED_CHANNELS) < original_len:
        await message.reply_text(f"✅ تم حذف القناة {channel_id} من قائمة الاشتراك الإجباري.")
    else:
        await message.reply_text("❌ لم يتم العثور على هذه القناة في القائمة.")
...
async def list_required_channels(client, message):
    ...
    if not REQUIRED_CHANNELS:
        await message.reply_text("ℹ️ قائمة الاشتراك الإجباري فارغة حالياً.")
        return
