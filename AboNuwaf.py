    ...
    if data == "clear_reviews":
        ...
        if not current:
            ...
            for t in reversed(sorted_times):
                if current_time >= t and t not in schedule_disabled and t not in ch_disabled_set:
                    current_station = AUTO_SCHEDULE[t]
                    break
            if not current_station:
                # خد آخر محطة مش معطّلة
                for t in reversed(sorted_times):
                    if t not in schedule_disabled and t not in ch_disabled_set:
                        current_station = AUTO_SCHEDULE[t]
                        break
            ch_info_now = user_data[user_id]["channels"][ch_id]
            # لو القناة متوقفة خارجياً (من تليجرام) — ما تشغلهاش تلقائياً
