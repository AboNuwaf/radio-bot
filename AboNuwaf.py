...
def save_required_channels():
    """حفظ القنوات في ملف احتياطي"""
    try:
        channels_json = json.dumps(REQUIRED_CHANNELS, ensure_ascii=False)
        os.environ["REQUIRED_CHANNELS"] = channels_json
...
def load_required_channels():
    """تحميل القنوات من Environment Variable أو الملف الاحتياطي"""
    global REQUIRED_CHANNELS
    try:
        val = os.environ.get("REQUIRED_CHANNELS", "[]").strip()
        if val and val != "[]":
            REQUIRED_CHANNELS = json.loads(val)
            return
...
def build_ffmpeg_cmd(input_url: str, output_url: str) -> list:
    """بناء أمر ffmpeg مع دعم SSL/TLS لروابط rtmps:// و User-Agent للـ radiojar"""
    cmd = ["ffmpeg"]

    # للروابط العادية نضيف reconnect
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        await query.answer()
        if query.from_user.id == OWNER_ID or has_perm(query.from_user.id, "view_reports"):
            # المطور والأدمن المخوّل يشوف البلاغات
            if not all_reports:
                await query.edit_message_text(
                ...
                text_msg = f"<blockquote>💬 كل البلاغات ({len(all_reports)} بلاغ)\n\n"
                ...
                if len(text_msg) > 4000:
                    ...
                    for chunk in chunks:
                        await query.message.reply_text(chunk)
                    await query.edit_message_text("<blockquote>💬 تم إرسال البلاغات أعلاه</blockquote>",
                        reply_markup=InlineKeyboardMarkup(buttons))
                else:
                    await query.edit_message_text(text_msg, reply_markup=InlineKeyboardMarkup(buttons))
        else:
        ...
        if query.from_user.id == OWNER_ID or has_perm(query.from_user.id, "view_reviews"):
            if not all_bot_reviews:
                await query.edit_message_text(
                    "<blockquote>⭐ تقييمات البوت\n\n📭 لا توجد تقييمات حتى الآن</blockquote>",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔙 رجوع", callback_data="support_back")]
                    ])
                )
            else:
                text_msg = f"<blockquote>⭐ كل تقييمات البوت ({len(all_bot_reviews)} تقييم)\n\n"
                for i, r in enumerate(all_bot_reviews, 1):
                    text_msg += (
                        f"{'─'*20}\n"
                        f"#{i} | {r['time']}\n"
                        f"👤 {r['name']} (@{r['username']})\n"
                        f"⭐ التقييم: {r['score']}/10\n"
                        f"💬 {r['comment']}\n\n"
                    )
                text_msg += "</blockquote>"
                buttons = [
                ...
                if len(text_msg) > 4000:
                    ...
                    for chunk in chunks:
                        await query.message.reply_text(chunk)
                    await query.edit_message_text("<blockquote>⭐ تم إرسال التقييمات أعلاه</blockquote>",
                        reply_markup=InlineKeyboardMarkup(buttons))
                else:
                    await query.edit_message_text(text_msg, reply_markup=InlineKeyboardMarkup(buttons))
        else:
        ...
        try:
            process = subprocess.Popen(
                ffmpeg_cmd,
                stdout=subprocess.DEVNULL,
                stderr=open("/data/ffmpeg_error.log", "a")
            )
...
@app.on_message(filters.command("testffmpeg") & filters.private)
