...
async def add_required_channel(client, message):
    if message.from_user.id != OWNER_ID:
        return
    # الصيغة: /add_channel @username رابط | نص مخصص (اختياري)
    parts = message.text.split(None, 3)
    if len(parts) < 3:
        await message.reply_text(
            "<blockquote>📢 إضافة قناة اشتراك إجباري\n\n"
            "الصيغة:\n"
            "/add_channel @username رابط\n\n"
            "أو مع نص مخصص:\n"
            "/add_channel @username رابط | نص رسالة الاشتراك</blockquote>"
        )
        return
    channel_id = parts[1]
    # فصل الرابط والنص المخصص
    rest = parts[2] if len(parts) > 2 else ""
    if " | " in rest:
        channel_url, custom_text = rest.split(" | ", 1)
        custom_text = custom_text.strip()
    else:
        channel_url = rest.strip()
        custom_text = ""
    for ch in REQUIRED_CHANNELS:
        if ch["id"] == channel_id:
            await message.reply_text("<blockquote>⚠️ هذه القناة موجودة بالفعل.</blockquote>")
            return
    REQUIRED_CHANNELS.append({"id": channel_id, "url": channel_url, "custom_text": custom_text})
    save_required_channels()
    txt = f"<blockquote>✅ تم إضافة {channel_id} بنجاح."
    if custom_text:
        preview = custom_text[:50] + "..." if len(custom_text) > 50 else custom_text
        txt += f"\n📝 النص المخصص: {preview}"
    txt += "</blockquote>"
    await message.reply_text(txt)


@app.on_message(filters.command("edit_channel") & filters.private)
async def edit_required_channel(client, message):
    if message.from_user.id != OWNER_ID:
        return
    # الصيغة: /edit_channel @username نص الرسالة الجديدة
    parts = message.text.split(None, 2)
    if len(parts) < 3:
        await message.reply_text(
            "<blockquote>✏️ تعديل نص قناة الاشتراك الإجباري\n\n"
            "الصيغة:\n"
            "/edit_channel @username النص الجديد\n\n"
            "لحذف النص المخصص والعودة للنص الافتراضي:\n"
            "/edit_channel @username -</blockquote>"
        )
        return
    channel_id = parts[1]
    ...
    if found:
        ...
        if new_text:
            ...
            await message.reply_text(f"<blockquote>✅ تم تحديث نص {channel_id}\n📝 النص الجديد: {preview}</blockquote>")
        else:
            await message.reply_text(f"<blockquote>✅ تم حذف النص المخصص من {channel_id} — سيُستخدم النص الافتراضي</blockquote>")
    else:
        await message.reply_text(f"<blockquote>❌ لم يتم العثور على {channel_id} في القائمة</blockquote>")

@app.on_message(filters.command("del_channel") & filters.private)
async def del_required_channel(client, message):
    if message.from_user.id != OWNER_ID:
        return
    args = message.text.split()
    if len(args) < 2:
        await message.reply_text("❌ الاستخدام: /del_channel @username")
        return
    channel_id = args[1]
    global REQUIRED_CHANNELS
    original_len = len(REQUIRED_CHANNELS)
    REQUIRED_CHANNELS = [ch for ch in REQUIRED_CHANNELS if ch["id"] != channel_id]
    if len(REQUIRED_CHANNELS) < original_len:
        save_required_channels()
        await message.reply_text(f"✅ تم حذف {channel_id} بنجاح.")
    else:
        await message.reply_text("❌ لم يتم العثور على هذه القناة.")
...
async def list_required_channels(client, message):
    ...
    if not REQUIRED_CHANNELS:
        await message.reply_text("ℹ️ قائمة الاشتراك الإجباري فارغة.")
        return
    text = "📋 قنوات الاشتراك الإجباري:\n\n"
    for i, ch in enumerate(REQUIRED_CHANNELS, 1):
        text += f"{i}- {ch['id']} ({ch['url']})\n"
