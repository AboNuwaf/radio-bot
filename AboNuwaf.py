...
def restore_broadcasts():
    ...
    for channel_id, info in saved_broadcasts.items():
        try:
            ...
            # لو القناة كانت متوقفة يدوياً قبل الـ restart — لا تعيد تشغيلها
            ...
            if not info.get("was_running", False):
                ...
                continue
            # لو user_data مافيهاش process ومفيش externally_stopped — يعني القناة كانت موقفة قبل الـ backup
            # نتحقق من schedule_enabled أو night_mode_enabled عشان نشغلها لو في جدول
            ...
            if "process" not in ch_data and not ch_has_schedule:
                logger.info(f"Skipping restore for {channel_id} — no active process in user_data")
                continue
