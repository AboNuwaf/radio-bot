        ...
        try:
            ...
            for uid, user_info in list(user_data.items()):
                ...
                for channel_id, channel_info in list(channels.items()):
                    # كل قناة لها night_mode_enabled مستقل جوّاها
...
def run_auto_schedule():
    ...
    while True:
        current_time = time.strftime("%H:%M", time.gmtime(time.time() + 7200))
        if current_time in AUTO_SCHEDULE and current_time not in schedule_disabled and last_triggered != current_time:
                            ...
                            if channel_info.get("broadcast_type") != "auto":
                                ...
                                try:
                                    asyncio.run_coroutine_threadsafe(
                                        app.send_message(
                                            int(uid),
                                            f"<blockquote>🤖 تم إيقاف البث اليدوي تلقائياً\n\n"
                                            f"📢 {ch_title}\n"
                                            f"🎙 وتم تشغيل: {station['name']}\n"
                                            f"📅 {egypt_strftime()}</blockquote>"
                                        ), _bot_loop
                                    )
                                except Exception:
            ...
            if switched:
                ...
                for uid, user_info in list(user_data.items()):
                    for ch_id, ch_info in user_info.get("channels", {}).items():
                        if ch_info.get("schedule_enabled", False) and "process" in ch_info:
...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        # night_toggle Global اتحذف — الوضع الليلي per-channel عبر unight_toggle
        await query.answer("❌ هذا الزر لم يعد متاحاً", show_alert=True)
        ...
        # عرض المواعيد مع حالة كل موعد لهذه القناة
        ...
        def to_ar_time(t):
            ...
            return f"{h12}:{m:02d} {period}"
        time_buttons = []
        for t, s in sorted(AUTO_SCHEDULE.items()):
            is_on = t not in ch_disabled
            name = s["name"].replace("إذاعة ", "").strip()
            icon = "✅" if is_on else "❌"
            time_buttons.append([
                InlineKeyboardButton(f"{icon} {to_ar_time(t)} — {name}",
                    callback_data=f"ch_sched_time_toggle_{ch_id}_{t}")
            ])
        await query.edit_message_text(
            f"<blockquote>📅 جدول القناة\n📢 {ch_title}\n\nالحالة: {status}\n\n"
            f"اضغط على موعد لتفعيله أو إيقافه لهذه القناة فقط:</blockquote>",
            reply_markup=InlineKeyboardMarkup(
                time_buttons + [
        ...
        if not current:
            ...
            sorted_times = sorted(AUTO_SCHEDULE.keys())
            for t in reversed(sorted_times):
                if current_time >= t:
                    current_station = AUTO_SCHEDULE[t]
                    break
            if not current_station and sorted_times:
                current_station = AUTO_SCHEDULE[sorted_times[-1]]
        ...
        broadcast_stats[channel_id] = {
            ...
            "user_id": user_id,
            "broadcast_type": user_data[user_id]["channels"][channel_id].get("broadcast_type", "manual")
        }
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            for channel_id, info in list(channels.items()):
                ...
                if is_ch_active:
                    ...
                    if not ch_station_name:
                        ch_station_url = info.get("station", user_data[user_id].get("temp_station", ""))
            ...
            if not broadcast_log:
                ...
                return
            lines = []
            for e in reversed(broadcast_log[-20:]):
                lines.append(
                    f"• {e['time']}\n"
                    f"  👤 {e['name']} ({e['username']})\n"
                    f"  📢 {e['channel']} — 🎙 {e['station']}"
                )
            text_log = "<blockquote>📋 سجل آخر البثوث\n\n" + "\n▬▬▬▬▬▬▬▬▬▬\n".join(lines) + "</blockquote>"
            await message.reply_text(text_log, reply_markup=admin_keyboard(user_id))
