...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        for i, ch in enumerate(REQUIRED_CHANNELS, 1):
            ...
            text += f"🆔 المعرف: {ch['id']}\n"
            text += f"━━━━━━━━━━━━━━━━\n"
            text += f"📊 الحالة: {'🟢 نشطة' if ch_url else '🔴 غير نشطة'}</blockquote>"
...
async def handle_text(client, message):
    ...
    try:
        ...
        if text == "إضافة قناة":
            ...
            REQUIRED_CHANNELS.append({"id": channel_id_add, "url": channel_url_add, "title": channel_title, "custom_text": "", "added_by": str(user_id)})
            save_required_channels()
            user_state.pop(user_id, None)
            await message.reply_text(
