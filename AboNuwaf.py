...
async def handle_callback(client, query):
    ...
    if data == "clear_reviews":
        ...
        parts = rest.split("_")
        uid = parts[0]
        # لو القسم الأخير مش رقم — هو اسم الكاتيجوري
        # نجمع الأجزاء اللي بعد uid ونحدد ch_id والكاتيجوري
        remaining = "_".join(parts[1:])
        # نشوف لو في ch_id (رقم) قبل الكاتيجوري
        ch_id_from_cat = None
        cat = remaining
        for cat_name in ST_CATEGORIES.keys():
            if remaining.endswith(cat_name):
                prefix = remaining[:-len(cat_name)].rstrip("_")
                if prefix and prefix.lstrip("-").isdigit():
                    ch_id_from_cat = prefix
                    cat = cat_name
                break
        await query.answer()
        from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
